...
#[derive(Default)]
pub struct BoltSubscription;

#[Subscription]
impl BoltSubscription {
    async fn query_app<'a>(
        ...
        let initial_response = BoltQuery::_query_app(&app_ctx.base, request.clone(), None).await?;
            ...
            .chain(
                stream
                    ...
                    .then(move |_block_height| {
                        ...
                        async move { BoltQuery::_query_app(&app_ctx.base, request, None).await }
                    }),
            ),
            sub_guard,
        ))
    ...
    async fn query_store<'a>(
        ...
        let initial_response =
            BoltQuery::_query_store(&app_ctx.base, key.clone(), None, prove).await?;
        let latest_block_height = initial_response.block_height;
        ...
        .chain(
            stream
                ...
                .then(move |_block_height| {
                    ...
                    async move { BoltQuery::_query_store(&app_ctx.base, key, None, prove).await }
                }),
        ), sub_guard))
    }
    ...
    async fn query_status<'a>(
        ...
        let stream = app_ctx.pubsub.subscribe().await?;
        let initial_response = BoltQuery::_query_status(&app_ctx.base).await;
        let latest_block_height = app_ctx.base.bolt_app.last_finalized_block().await?.height;
            ...
            .chain(
                stream
                    ...
                    .then(move |_| {
                        ...
                        async { BoltQuery::_query_status(&app_ctx.base).await }
                    }),
            ),
            sub_guard,
        ))
