use {
    ...
    velox_testing::{Preset, TestOption},
    velox_types::constants::usdc,
    graphql_client::GraphQLQuery,
    bolt::{BlockCreation, Coins, MOCK_CHAIN_ID, Message, NonEmpty, ResultExt, Signer},
    bolt_types::{BroadcastClientExt, Denom, GasOption, Inner, JsonSerExt},
    indexer_sql::entity,
    indexer_testing::{
...
async fn graphql_subscribe_to_transactions() -> anyhow::Result<()> {
    ...
    // Use typed subscription from velox-graphql-types
    ...
    tokio::spawn(async move {
        while rx.recv().await.is_some() {
            ...
            client
                .send_message(
                    &mut accounts["sender"],
                    Message::transfer(to, Coins::one(Denom::from_str("ubolt")?, 2_000)?)?,
...
async fn transactions_stores_httpd_details() -> anyhow::Result<()> {
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            if let Err(error) = velox_mock_httpd::run_with_callback(
                port,
