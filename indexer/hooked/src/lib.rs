use {
    ...
    bolt_app::{APP_CONFIG, CONFIG, Indexer, IndexerResult, LAST_FINALIZED_BLOCK},
    bolt_types::{Config, Json},
    std::{
        collections::HashMap,
        sync::{
            Arc,
...
impl HookedIndexer {
    ...
    pub async fn add_indexer<I>(&mut self, indexer: I) -> Result<&mut Self, bolt_app::IndexerError>
    where
        I: Indexer + Send + Sync + 'static,
    {
        if self.is_running.load(Ordering::Relaxed) {
            return Err(bolt_app::IndexerError::already_running());
        }
    ...
    pub async fn reindex(&self, storage: &dyn bolt_types::Storage) -> IndexerResult<()> {
        match LAST_FINALIZED_BLOCK.load(storage) {
            ...
            Ok(block) => {
                ...
                let cfg = CONFIG.load(storage).map_err(|e| {
                    bolt_app::IndexerError::storage(format!("Failed to load CONFIG: {e}"))
                })?;

                let app_cfg = APP_CONFIG.load(storage).map_err(|e| {
                    bolt_app::IndexerError::storage(format!("Failed to load APP_CONFIG: {e}"))
                })?;
                ...
                for block_height in (min_height + 1)..=block.height {
                    let mut ctx = bolt_app::IndexerContext::new();
                    ...
                    if !errors.is_empty() {
                        return Err(bolt_app::IndexerError::multiple(errors));
                    }
                }
            },
        }
...
impl Indexer for HookedIndexer {
    /// Start all indexers
    #[cfg_attr(feature = "tracing", tracing::instrument(skip_all))]
    async fn start(&mut self, storage: &dyn bolt_types::Storage) -> IndexerResult<()> {
        if self.is_running.load(Ordering::Relaxed) {
            return Err(bolt_app::IndexerError::already_running());
        }
    ...
    async fn index_block(
        &self,
        block: &bolt_types::Block,
        block_outcome: &bolt_types::BlockOutcome,
        ctx: &mut bolt_app::IndexerContext,
    ) -> IndexerResult<()> {
        if !self.is_running.load(Ordering::Relaxed) {
            return Err(bolt_app::IndexerError::not_running());
...
#[cfg(test)]
mod tests {
    use {super::*, bolt_types::MockStorage};
    ...
    impl Indexer for TestIndexer {
        async fn start(&mut self, _storage: &dyn bolt_types::Storage) -> IndexerResult<()> {
        ...
        async fn pre_indexing(
            &self,
            _block_height: u64,
            _ctx: &mut bolt_app::IndexerContext,
        ) -> IndexerResult<()> {
        ...
        async fn index_block(
            &self,
            _block: &bolt_types::Block,
            _block_outcome: &bolt_types::BlockOutcome,
            _ctx: &mut bolt_app::IndexerContext,
        ) -> IndexerResult<()> {
    ...
    async fn test_operations_when_not_running() {
        ...
        let block = bolt_types::Block {
            info: bolt_types::BlockInfo {
                height: 1,
                timestamp: bolt_types::Timestamp::from_seconds(1),
                hash: [0u8; 32].into(),
            },
            txs: vec![],
        };

        let outcome = bolt_types::BlockOutcome {
            height: 1,
            app_hash: bolt_types::Hash256::ZERO,
            cron_outcomes: vec![],
            tx_outcomes: vec![],
        };
    ...
    async fn test_context_data_passing() {
        ...
        let block = bolt_types::Block {
            info: bolt_types::BlockInfo {
                height: 42,
                timestamp: bolt_types::Timestamp::from_seconds(123456789),
