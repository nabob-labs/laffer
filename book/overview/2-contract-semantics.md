...
This chapter documents the programming model for Velox smart contracts: entry points,
...
Entry points are defined using the `#[velox_ffi::export]` attribute macro, which generates
...
targeting the **RustVm** (all first-party Velox contracts) do not need it -- they
...
```rust
// velox/core/types/src/context.rs
...
`velox/core/math/` provides overflow-safe fixed-point arithmetic:
...
Velox extends the base math types with `Number<Q, U, D>`
(`velox/exchange/types/src/typed_number.rs`), a **dimensionally-typed** signed fixed-point
...
Velox encourages declarative validation via `Bounded<T, B>` and `LengthBounded<T>`:
...
There is also `StorageQuerier::query_wasm_path` (`velox/core/storage/src/querier.rs`), which
...
This is the preferred query method in Velox's inter-contract calls (e.g., the auth
...
Velox uses **account abstraction** -- every user has a dedicated smart contract instance
...
Instead, Velox tracks the **most recent 20 nonces seen** (`SEEN_NONCES`). A new tx
is accepted if:
...
`velox/core/ffi/` bridges WASM guests and the host:
...
`velox/core/testing/` provides a high-level integration test harness:
...
### Velox-specific test suite

`velox/testing/` extends the base suite with helpers for deploying the full Velox
...
`velox/core/macros/` provides:

- **`#[velox_ffi::export]`** -- Generates WASM FFI wrappers for entry points. Only needed
  for WasmVm contracts; RustVm contracts register entry points directly.
- **`#[velox_primitives::derive(Serde, Borsh)]`** -- Derives standard traits (Serialize,
  Deserialize, BorshSerialize, BorshDeserialize, Clone, Debug, PartialEq, Eq).
- **`#[velox_primitives::event("name")]`** -- Registers an event type with a canonical name.
- **`#[velox_storage::index_list(PK, T)]`** -- Implements `IndexList` trait for IndexedMap
  secondary indexes.
