...
Velox is a custom blockchain state machine that runs on top of CometBFT consensus.
...
Velox separates storage into two independent stores following the Cosmos SDK ADR-065
pattern:
...
```rust
// velox/core/db/memory/src/db.rs
...
```rust
// velox/core/app/src/traits/db.rs
pub trait Db {
...
(`velox/core/jellyfish-merkle/`). The tree provides:
...
```rust
// velox/core/jellyfish-merkle/src/proof.rs
...
```rust
// velox/core/app/src/traits/commitment.rs
...
`velox/core/storage/` provides type-safe, namespace-aware abstractions over raw key-value
storage.
...
Each contract's storage is wrapped in a `StorageProvider` (`velox/core/app/src/providers/storage.rs`):
...
The `App` struct (`velox/core/app/src/app.rs`) is the state machine's entry point. It
...
```rust
// velox/core/types/src/buffer.rs
pub struct Buffer<S> {
    base: S,
...
```rust
// velox/core/app/src/gas/tracker.rs
...
Gas costs (`velox/core/app/src/gas/costs.rs`):
...
```rust
// velox/core/app/src/traits/vm.rs
pub trait Vm: Sized {
...
**Gatekeeper middleware** (`velox/core/vm/wasm/src/gatekeeper.rs`): Validates WASM
...
**Host functions** (`velox/core/vm/wasm/src/imports.rs`): The WASM guest can call these
