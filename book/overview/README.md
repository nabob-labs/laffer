...
3. **[Velox Contract System](3-velox-contracts.md)** -- Each smart contract (bank, accounts,
...
┌──────────────────────────────────────────────────────────────┐
│  TRUSTED: Node Binary                                        │
│   velox/core/app (ABCI + state transitions)                  │
│   velox/core/db  (RocksDB persistence)                       │
│   velox/core/vm/rust (native contract execution, no sandbox) │
│   velox/core/jellyfish-merkle (state commitment)             │
│   velox/* system contracts (bank, accounts, etc.)            │
│   velox/indexer/* (read-only; cannot affect consensus)       │
└────────────────────── ▼ ─────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│  UNTRUSTED: Third-Party WASM Contracts                       │
│   Executed inside velox/core/vm/wasm (Wasmer sandbox)        │
...
> **Note:** In the current Velox deployment, all contracts are first-party and executed
