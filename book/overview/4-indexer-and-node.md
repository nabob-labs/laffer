...
This chapter covers the indexer pipeline, SQL schema, GraphQL API, and the Velox CLI
...
```rust
// velox/core/app/src/traits/indexer.rs
#[async_trait]
...
`velox/indexer/hooked/` is the single `Indexer` impl that the chain wires into `App`. It owns the three production indexer components by value and orchestrates their per-block work:

```rust
pub struct HookedIndexer {
    pub file:       velox_indexer_cache::Cache,
    pub sql:        velox_indexer_sql::Indexer,
    pub clickhouse: velox_indexer_clickhouse::Indexer,
...
## 2. SQL Indexer (`velox/indexer/sql/`)
...
An in-memory ring buffer of recent block events (`velox/indexer/sql/src/event_cache.rs`).
Configurable window size. Used for fast GraphQL lookups without DB round-trips.

## 3. Cache Indexer (`velox/indexer/cache/`)

Persists complete block + outcome data to disk for recovery:

```text
~/.velox/indexer/blocks/{height}.json  -- Serialized block data
~/.velox/indexer/last_block.json       -- Latest block height
```

## 4. Velox-Specific Writes (`velox/indexer/sql/src/write/`)

The SQL indexer crate also performs Velox-specific data extraction in the same `post_indexing` pass, after the generic block/tx/message/event rows have been written:
...
## 5. GraphQL / HTTP Server (`velox/indexer/httpd/`)
...
## 6. Node Startup (`velox/cli/`)

The `velox start` command initializes and runs the full node:
...
7.  Setup indexer stack (HookedIndexer with three components):
    ...
    ├── SqlIndexer (PostgreSQL — generic + Velox-specific tables)
    └── ClickhouseIndexer (analytics)
8.  Run DB migrations + catch-up reindexing
9.  Spawn:
    ├── Velox HTTP server (GraphQL)
...
│  Cache → disk                                          │
│  SqlIndexer → PostgreSQL (generic + Velox tables)      │
...
| Velox GraphQL | 8000         | Configurable                    |
