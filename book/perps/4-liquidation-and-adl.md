...
All eight numbered examples — plus mirrored variants with the sides flipped — are implemented as end-to-end tests in [`velox/testing/tests/perps/liquidation_spec.rs`](https://github.com/nabob-labs/laffer/blob/main/velox/testing/tests/perps/liquidation_spec.rs), asserting every figure below exactly. The negative-margin edge case is likewise covered by an end-to-end test there.
