...
Every Velox deployment exposes its data and trading surface over two HTTP servers:
...
A quick mental model for readers coming from other venues: `POST /query` is the universal read (Velox's analogue of Hyperliquid's `/info`), and the `GET /perps/*` and `GET /account/*` routes are typed shortcuts over it; `POST /broadcast` is the universal write (the analogue of Hyperliquid's `/exchange`).
...
The perps exchange's numeric types are **signed fixed-point decimals with 6 decimal places**, built on [`velox_types::Number`](https://github.com/nabob-labs/laffer/blob/main/velox/exchange/types/src/typed_number.rs). They are serialized as **strings** to avoid floating-point loss:
...
`Addr` and `Hash256` use different text dialects — an address is lowercase and `0x`-prefixed, a hash is bare uppercase. EVM tooling typically displays hashes (e.g. Hyperlane message IDs) as `0x`-prefixed lowercase; strip the prefix and uppercase the rest before passing one to Velox, or the request fails to deserialize.
...
```json
{
  ...
  "data": { "user_index": 0, "chain_id": "velox-1", "nonce": 42, "expiry": null },
  "credential": { /* … */ }
}
```
...
```json
{
  ...
  "gas_limit": 1500000,
  "messages": [ /* … */ ],
  "data": { "chain_id": "velox-1", "expiry": null, "nonce": 42, "user_index": 0 }
}
```
...
curl -X POST https://<live-host>/simulate \
  -H 'Content-Type: application/json' \
  -d '{"sender": "0x1234…abcd", "msgs": [ /* … */ ], "data": {"user_index": 0, "chain_id": "velox-1", "nonce": 42, "expiry": null}}'
```
...
The master account is created **inactive** (spam prevention); the new address is returned in the transaction events. **Second**, send it at least the `minimum_deposit` (10 USDC = `10000000` `bridge/usdc` on mainnet), from an existing Velox account or bridged in via Hyperlane, and the account activates on receipt. To confirm a bridged deposit arrived, query the mailbox's `delivered` method with the Hyperlane message id via `POST /query` (`Hash256` uppercase, no `0x`); a `true` result is permanent and means the funds are spendable.
...
The response objects of the perps [read shortcuts](#32-typed-read-shortcuts) mirror the contract types in [`velox/exchange/types/src/perps.rs`](https://github.com/nabob-labs/laffer/blob/main/velox/exchange/types/src/perps.rs), the authoritative source. The consumer-facing fields are documented below; the global-parameter structs also carry vault-market-making and governance knobs, elided here and marked in the source.
