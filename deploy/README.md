...
This directory contains Ansible playbooks and roles for deploying the Velox
...
| -------------- | --------------------------------------------------------------- | --------------------- |
| velox          | Blockchain node / API server, connects to postgres + clickhouse |                       |
| cometbft       | BFT consensus engine, P2P between nodes via WireGuard           |                       |
| velox-frontend | Web UI                                                          |                       |
...
flowchart TD
    ...
    class Velox,CometBFT,Frontend,GraphiQL,PointsBot app
...
- **Traces**: velox emits OTLP -> Tempo (ovh3) -> Grafana
...
cosign verify \
  ...
  --certificate-identity-regexp "https://github.com/nabob-labs/laffer/.github/workflows/rust.yml@refs/heads/main" \
  ghcr.io/nabob-labs/laffer/velox@sha256:<digest>
```
