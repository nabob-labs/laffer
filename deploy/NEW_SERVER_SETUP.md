...
Application-level scrapes — `traefik`, `clickhouse`, `postgres`, `velox`, `cometbft`, `hyperlane-validator`, `perp-liquidator` — need their own entries in `prometheus.yml` once you deploy those services to the host. See the existing entries in the same file for the format and labels.
