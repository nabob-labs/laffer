...
`ghcr.io/nabob-labs/laffer/hyperlane-agents`) as standalone docker-compose
stacks, one instance per host. They are NOT CometBFT validators — they are the
interchain-messaging agents that move Warp-route messages between Velox and the
EVM chains.

- **Validator** — watches the Velox mailbox (`originchainname: velox[testnet]`)
  and signs merkle checkpoints. Covers the **withdraw** direction (Velox → EVM).
...
Worked example: `arbitrum` (Arbitrum One, domain `42161`). Velox domains:
...
3. **`files/relayer-<net>-config.json`** — add to `relaychains` and the
   `whitelist` (both directions, velox ↔ new chain):
   ```json
   "relaychains": "velox,ethereum,arbitrum",
   "whitelist": [
     ...,
...
For `<chain> → velox` (deposits) the **Velox ISM** must trust validator(s)
...
the Sepolia validator was down). Our deploy only runs **velox-origin**
validators (withdraw direction); EVM-origin validators come from elsewhere.

## Deploy & verify

```bash
cd ~/github/nabob-labs/laffer/deploy
