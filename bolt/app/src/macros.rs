...
macro_rules! catch_and_update_event {
    ($result:expr, $evt:expr => $field:ident) => {
        match $result {
            EventResult::Ok(i) => {
                $evt.$field = bolt_types::EventStatus::Ok(i);
            },
            EventResult::Err { event, error } => {
                $evt.$field = bolt_types::EventStatus::Failed {
                    event,
            ...
            EventResult::NestedErr { event, error } => {
                $evt.$field = bolt_types::EventStatus::NestedFailed(event);
...
macro_rules! catch_and_push_event {
    ($result:expr, $evt:expr, $field:ident) => {
        match $result {
            EventResult::Ok(i) => {
                $evt.$field.push(bolt_types::EventStatus::Ok(i));
            },
            EventResult::Err { event, error } => {
                $evt.$field.push(bolt_types::EventStatus::Failed {
                    event,
                    error: error_backtrace::Backtraceable::into_generic_backtraced_error(error.clone()),
                });
            ...
            EventResult::NestedErr { event, error } => {
                $evt.$field.push(bolt_types::EventStatus::NestedFailed(event));
...
macro_rules! catch_and_insert_event {
    ($result:expr, $evt:expr, $field:ident, key:$key:expr) => {
        match $result {
            EventResult::Ok(i) => {
                $evt.$field.insert($key, bolt_types::EventStatus::Ok(i));
            },
            EventResult::Err { event, error } => {
                $evt.$field.insert($key, bolt_types::EventStatus::Failed {
                    event,
                    error: error_backtrace::Backtraceable::into_generic_backtraced_error(error.clone()),
                });
            ...
            EventResult::NestedErr { event, error } => {
                $evt.$field.insert($key, bolt_types::EventStatus::NestedFailed(event));
