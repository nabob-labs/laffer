...
use {
    ...
    bolt_types::{Batch, Hash256, Storage},
};
...
/// (e.g. using `bolt_db::Buffer`) and then use the `flush` and `commit`
...
pub trait IbcDb: Db {
    ...
    /// ## Note
    ///
    /// This needs to be implemented at the `Db` level, instead of in bolt-jmt,
    /// because ICS-23 requires proofs to contain the prehash key and value,
    /// while bolt-jmt only store hashed keys and values. Therefore we need the
    /// state storage.
    fn ics23_prove(
        &self,
        key: Vec<u8>,
