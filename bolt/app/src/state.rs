use {
    bolt_storage::{Index, IndexList, IndexedMap, Item, Map, MultiIndex, Set},
    bolt_types::{
...
pub const BOLT_NAMESPACE_LEN: usize = 4;

/// Check at compile time that all namespaces are of the correct length.
const fn bolt_namespace(s: &str) -> &str {
    assert!(
        s.len() == BOLT_NAMESPACE_LEN,
        "bolt storage namespaces must be exactly 4 bytes long"
    );
...
pub const CHAIN_ID: Item<String> = Item::new(bolt_namespace("chid"));

/// The most recently finalized block
pub const LAST_FINALIZED_BLOCK: Item<BlockInfo> = Item::new(bolt_namespace("lfbk"));

/// Chain-level configuration
pub const CONFIG: Item<Config> = Item::new(bolt_namespace("cnfg"));

/// Application-specific configuration.
pub const APP_CONFIG: Item<Json> = Item::new(bolt_namespace("acfg"));

/// Scheduled cronjobs.
///
/// This needs to be a `Set` instead of `Map<Timestamp, Addr>` because there can
/// be multiple jobs with the same scheduled time.
pub const NEXT_CRONJOBS: Set<(Timestamp, Addr)> = Set::new(bolt_namespace("crjb"));

/// A chain upgrade that is scheduled to happen in a future block.
pub const NEXT_UPGRADE: Item<NextUpgrade> = Item::new(bolt_namespace("nxup"));

/// Chain upgrades that have been carried out in the past.
pub const PAST_UPGRADES: Map<u64, PastUpgrade> = Map::new(bolt_namespace("pvup"));

/// Wasm contract byte codes: code_hash => byte_code
pub const CODES: IndexedMap<Hash256, Code, CodeIndexes> =
    IndexedMap::new(bolt_namespace("code"), CodeIndexes {
...
pub const CONTRACTS: Map<Addr, ContractInfo> = Map::new(bolt_namespace("ctrt"));
    ...
    fn codes() {
        ...
        // Find _all_ codes, regardless of orphaned or in use.
        {
            let res = CODES
                .idx
                .status
                .prefix_keys(&storage, None, None, bolt_types::Order::Ascending)
