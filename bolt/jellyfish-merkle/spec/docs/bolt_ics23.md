...
> [!TIP]
> This markdown file contains some metadata and comments that enable it to be tangled to a full Quint file (using [lmt](https://github.com/driusan/lmt)). The Quint file can be found at [bolt_ics23.qnt](../quint/bolt_ics23.qnt).

<!-- Boilerplate: tangled from comment to avoid markdown rendering
```bluespec quint/bolt_ics23.qnt
// -*- mode: Bluespec; -*-

// This is a protocol specification of ICS23, tuned towards the Bolt JMT
...
```
-->
We created a variable with the same values that were set in the [following Rust code](https://github.com/nabob-labs/laffer/blob/7a0451dfad14d607722c33bec20ae56dd5c3bffa/bolt/jellyfish-merkle/src/ics23.rs#L18-L37):
...
`InnerSpec.child_order` for Bolt JMT is defined [here](https://github.com/nabob-labs/laffer/blob/4890cb58a4b60acfacbc19d6c041f72db2b5b8ee/bolt/jellyfish-merkle/src/ics23.rs#L27).
...
First it calculates hash of the leaf based on `key` and `value` from the proof. To do so, we are using a specific hash function from Bolt JMT implementation.
