use {
    ...
    bolt_app::{AppError, GasTracker, Instance, QuerierProvider, StorageProvider, Vm},
    bolt_types::{Context, Hash256},
    bolt_vm_rust::{RustInstance, RustVm},
    bolt_vm_wasm::{WasmInstance, WasmVm},
...
pub enum VmError {
    ...
    Rust(bolt_vm_rust::VmError),

    #[error("WasmVm error: {0}")]
    Wasm(bolt_vm_wasm::VmError),
}
...
mod _testing {
    use {
        crate::HybridVm,
        bolt_testing::TestVm,
        bolt_types::{Binary, Hash256, HashExt},
        bolt_vm_rust::RustVm,
    ...
    impl bolt_testing::TestVm for HybridVm {
        fn default_account_code() -> bolt_types::Binary {
            DEFAULT_ACCOUNT_CODE.clone()
        }

        fn default_bank_code() -> bolt_types::Binary {
            DEFAULT_BANK_CODE.clone()
        }

        fn default_taxman_code() -> bolt_types::Binary {
