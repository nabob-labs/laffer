use {
    ...
    bolt_testing::{TestAccounts, TestBuilder, TestSuite},
    bolt_types::{Addr, Binary, Coins, Denom, HashExt, QuerierExt, Query, ResultExt},
    bolt_vm_hybrid::HybridVm,
...
pub fn setup_test() -> (TestSuite<MemDb, HybridVm>, TestAccounts, Addr, Addr) {
    let rust_tester: Binary = ContractBuilder::new(Box::new(bolt_tester::instantiate))
        .with_query(Box::new(bolt_tester::query))
        .build()
        .to_bytes()
        .into();
    ...
    let wasm_tester = suite
        .upload_and_instantiate_with_gas(
            &mut accounts["sender"],
            320_000_000,
            read_wasm_file("bolt_tester.wasm"),
            &bolt_tester::InstantiateMsg {},
            "tester",
            Some("tester"),
            None,
            Coins::new(),
...
fn backtrace() {
    ...
    // there are not any bolt_tester function name in the backtrace.
    assert!(!backtrace.is_empty());
    assert!(!backtrace.contains("bolt_tester::query::failing_query"));
