use {
    ...
    async_graphql::*,
    bolt_types::{Binary, Inner, QueryResponse, TxOutcome},
    std::str::FromStr,
};
#[cfg(feature = "metrics")]
...
pub struct BoltQuery {}

impl BoltQuery {
    pub async fn _query_app(
        app_ctx: &crate::context::Context,
        request: bolt_types::Query,
        height: Option<u64>,
    ) -> Result<QueryResponseWithBlockHeight, Error> {
        #[cfg(feature = "metrics")]
        let start = Instant::now();

        let (response, block_height) = app_ctx.bolt_app.query_app(request, height).await?;

        #[cfg(feature = "metrics")]
        histogram!("http.bolt.query_app.duration").record(start.elapsed().as_secs_f64());
    ...
    pub async fn _query_store(
        ...
        #[cfg(feature = "metrics")]
        histogram!("http.bolt.query_store.duration").record(start.elapsed().as_secs_f64());
    ...
    pub async fn _query_status(app_ctx: &crate::context::Context) -> Result<Status, Error> {
        ...
        let status = Status {
            block: app_ctx.bolt_app.last_finalized_block().await?.into(),
            chain_id: app_ctx.bolt_app.chain_id().await?,
        };

        #[cfg(feature = "metrics")]
        histogram!("http.bolt.query_status.duration").record(start.elapsed().as_secs_f64());
...
impl BoltQuery {
    async fn query_app(
        ...
        #[graphql(desc = "Request as JSON")] request: bolt_types::Query,
        height: Option<u64>,
    ...
    async fn simulate(
        &self,
        ctx: &async_graphql::Context<'_>,
        #[graphql(desc = "Transaction as Json")] tx: bolt_types::UnsignedTx,
    ) -> Result<TxOutcome, Error> {
        let app_ctx = ctx.data::<crate::context::Context>()?;

        Ok(app_ctx.bolt_app.simulate(tx).await?)
    }
}
