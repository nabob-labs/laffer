...
use {
    ...
    bolt_math::{Dec, Int, IsZero, MathResult, Number, NumberConst, Uint128},
...
impl Duration {
    ...
    /// Convert the `bolt::Duration` to a `std::time::Duration`.
    ///
    /// ## Panics
    ///
...
#[cfg(feature = "chrono")]
impl Timestamp {
    /// Convert the `bolt::Timestamp` to a `chrono::DateTime<Utc>`.
    ///
    /// ## Panics
    ///
