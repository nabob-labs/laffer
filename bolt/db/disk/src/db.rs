...
use uuid::Uuid;
#[cfg(feature = "metrics")]
use {crate::statistics, bolt_types::MetricsIterExt};
use {
    crate::{DbError, DbResult},
    bolt_app::{CONTRACT_NAMESPACE, Commitment, Db, BOLT_NAMESPACE_LEN},
    bolt_types::{Addr, Batch, Buffer, Hash256, HashExt, Op, Order, Record, Storage},
    itertools::Itertools,
...
#[cfg(feature = "metrics")]
pub const DISK_DB_LABEL: &str = "bolt.db.disk.duration";

#[cfg(feature = "metrics")]
pub const ITERATOR_DB_LABEL: &str = "bolt.db.disk.iterator.count";
    ...
    T: Commitment,
{
    ...
    fn flush_but_not_commit(&self, batch: Batch) -> DbResult<(u64, Option<Hash256>)> {
        ...
        // We assume Bolt App has the following behavior: for each key that is
...
pub fn new_storage_cf_options(mut opts: Options) -> Options {
    ...
    opts.set_prefix_extractor(SliceTransform::create_fixed_prefix(BOLT_NAMESPACE_LEN));
    opts
}
...
pub fn new_storage_read_options(
    ...
    if let (Some(min), Some(max)) = (min, max)
        && min.len() >= BOLT_NAMESPACE_LEN
        && max.len() >= BOLT_NAMESPACE_LEN
        && min[..BOLT_NAMESPACE_LEN] == max[..BOLT_NAMESPACE_LEN]
    {
        opts.set_prefix_same_as_start(true);
    }
    opts
...
mod tests_jmt {
    use {
        crate::DiskDb,
        bolt_app::Db,
        bolt_jmt::{MerkleTree, verify_proof},
        bolt_types::{
    ...
    #[test]
    fn disk_db_works() {
        let path = TempDataDir::new("_bolt_disk_db_works");
    ...
    fn disk_db_pruning_works() {
        let path = TempDataDir::new("_bolt_disk_db_pruning_works");
        let db = DiskDb::<MerkleTree>::open(&path).unwrap();
        ...
        for version in 0..=2 {
            ...
            assert!(db.prove(b"a", Some(version)).is_err_and(|err| {
                err.to_string()
                    .contains("data not found! type: bolt_jmt::node::Node")
            }));
        }
...
mod tests_simple {
    use {
        super::*,
        bolt_app::{SimpleCommitment, StorageProvider},
        bolt_storage::Map,
        bolt_types::{BorshSerExt, MockStorage, ResultExt, Shared, btree_map, btree_set, hash},
        temp_rocksdb::TempDataDir,
    };
    ...
    fn disk_db_lite_works() {
        let path = TempDataDir::new("_bolt_disk_db_lite_works");
    ...
    #[test]
    fn priority_data() {
        let path = TempDataDir::new("_bolt_disk_db_priority_data");
    ...
    fn priority_data_new_db() {
        let path = TempDataDir::new("_bolt_disk_db_priority_data_new_db");
    ...
    fn rejecting_non_wasm_key() {
        ...
        let path = TempDataDir::new("_bolt_disk_db_lite_iterator_works");
        let db = DiskDb::<SimpleCommitment>::open(&path).unwrap();
...
mod test_deadlock {
    use {
        super::*,
        bolt_app::SimpleCommitment,
        bolt_storage::{Index, IndexList, IndexedMap, MultiIndex},
        std::time::Duration,
        temp_rocksdb::TempDataDir,
    };
    ...
    fn run_deadlock_test() {
        let path = TempDataDir::new("_bolt_disk_db_deadlock_problem");
