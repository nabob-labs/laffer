use {
    bolt_types::{Addr, ByteArray, JsonSerExt, Message, StdError, StdResult},
...
impl bolt_types::SignData for SignDoc<'_> {
    type Error = StdError;
    type Hasher = Sha256;
    ...
    fn to_prehash_sign_data(&self) -> StdResult<Vec<u8>> {
        ...
        // into JSON. Here we use `bolt::to_json_vec` as the source of truth.
...
/// Schema for the account credentials expected in [`Tx::credential`](bolt_types::Tx::credential).
