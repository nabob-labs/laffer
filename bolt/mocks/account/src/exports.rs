use {
    ...
    bolt_types::{ImmutableCtx, Json, JsonSerExt, MutableCtx, Response, StdResult},
};

// Need to define these manually because we can't use the `bolt::export` macro in
...
mod __wasm_exports {
    ...
    extern "C" fn instantiate(ctx_ptr: usize, msg_ptr: usize) -> usize {
        bolt_ffi::do_instantiate(&crate::instantiate, ctx_ptr, msg_ptr)
    }

    #[unsafe(no_mangle)]
    extern "C" fn authenticate(ctx_ptr: usize, tx_ptr: usize) -> usize {
        bolt_ffi::do_authenticate(&crate::authenticate, ctx_ptr, tx_ptr)
    }

    #[unsafe(no_mangle)]
    extern "C" fn receive(ctx_ptr: usize) -> usize {
        bolt_ffi::do_receive(&super::receive, ctx_ptr)
    }

    #[unsafe(no_mangle)]
    extern "C" fn execute(ctx_ptr: usize, msg_ptr: usize) -> usize {
        bolt_ffi::do_execute(&super::execute, ctx_ptr, msg_ptr)
    }

    #[unsafe(no_mangle)]
    extern "C" fn query(ctx_ptr: usize, msg_ptr: usize) -> usize {
        bolt_ffi::do_query(&super::query, ctx_ptr, msg_ptr)
    }
}
