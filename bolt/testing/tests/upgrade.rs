use {
    bolt_app::{AppError, CHAIN_ID, CONFIG, CONTRACTS, GasTracker, HaltReason, TraceOption},
    bolt_math::{Bytable, NextNumber, Uint128, Uint256},
...
fn upgrading_with_calling_contract() {
    mod new_mock_bank {
        use {
            bolt_math::{NextNumber, Uint256},
            bolt_storage::Map,
            bolt_types::{Addr, Denom, Empty, MutableCtx, Order, Response, StdResult},
        };
        ...
        pub fn execute(ctx: MutableCtx, _msg: Empty) -> StdResult<Response> {
            ...
            // Do not confuse:
            // - The old map is `bolt_mock_bank::BALANCES_BY_ADDR`.
            // - The new map is `new_mock_bank::BALANCES_BY_ADDR`.
            for ((addr, denom), amount) in bolt_mock_bank::BALANCES_BY_ADDR
    ...
    suite
        .query_wasm_raw(
            bank,
            bolt_mock_bank::BALANCES_BY_ADDR.path((accounts["owner"].address, &denom)),
        )
    ...
    suite.app.set_cargo_version_and_upgrade_handler(
        ...
        Some(|mut storage, vm, block| {
            ...
            let new_mock_bank_code = ContractBuilder::new(Box::new(bolt_mock_bank::instantiate))
                .with_execute(Box::new(new_mock_bank::execute))
                .build();

            // Update the bank contract's code.
            bolt_app::CODES.update(&mut storage, bank_contract.code_hash, |mut code| {
