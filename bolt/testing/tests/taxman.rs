use {
    ...
    bolt_types::{Coins, Empty, Message, ResultExt},
...
mod taxman {
    use {
        bolt_math::{IsZero, MultiplyFraction, Number, NumberConst, Udec128, Uint128},
        bolt_types::{
    ...
    pub static FEE_DENOM: LazyLock<Denom> = LazyLock::new(|| Denom::from_str("ubolt").unwrap());
    ...
    pub fn withhold_fee(ctx: AuthCtx, tx: Tx) -> StdResult<Response> {
        ...
        let withhold_msg = if withhold_amount.is_non_zero() {
            Some(Message::execute(
                bank,
                &bolt_mock_bank::ExecuteMsg::ForceTransfer {
                    from: tx.sender,
                    to: ctx.contract,
