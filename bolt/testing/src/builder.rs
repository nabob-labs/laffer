...
pub const DEFAULT_FEE_DENOM: &str = "ubolt";
...
pub struct TestBuilder<
    ...
    ID = NullIndexer,
    M1 = bolt_mock_account::InstantiateMsg,
    M2 = bolt_mock_bank::InstantiateMsg,
    M3 = bolt_mock_taxman::InstantiateMsg,
    OW = Undefined<Addr>,
...
> {
    ...
    owner: OW,
    // Accounts
    account_opt: CodeOption<Box<dyn Fn(bolt_mock_account::PublicKey) -> M1>>,
    accounts: TA,
    // Bank
    ...
    pub fn new_with_vm_and_pp_and_indexer(db: DB, vm: VM, pp: PP, indexer: ID) -> Self {
        Self {
            ...
            account_opt: CodeOption {
                ...
                msg_builder: Box::new(|public_key| bolt_mock_account::InstantiateMsg {
                    public_key,
                }),
            },
            bank_opt: CodeOption {
                code: VM::default_bank_code(),
                msg_builder: Box::new(|initial_balances| bolt_mock_bank::InstantiateMsg {
                    initial_balances,
                }),
            },
            taxman_opt: CodeOption {
                code: VM::default_taxman_code(),
                msg_builder: Box::new(|fee_denom, fee_rate| bolt_mock_taxman::InstantiateMsg {
                    config: bolt_mock_taxman::Config {
                        fee_denom,
                        fee_rate,
                    },
                }),
    ...
    AppError: From<VM::Error>,
{
    ...
    /// provided by the Bolt test suite.
    ///
    ...
    ///     bolt_types::Coins,
    ///     bolt_vm_rust::ContractBuilder,
    /// };
    ///
    /// let code = ContractBuilder::new(Box::new(bolt_mock_bank::instantiate))
    ...
    ///     .set_bank_code(
    ///         code,
    ///         |initial_balances| bolt_mock_bank::InstantiateMsg { initial_balances },
    ///     )
    ///     .build();
    /// ```
    ...
    /// };
    ///
    /// let code = ContractBuilder::new(Box::new(bolt_mock_taxman::instantiate))
    ...
    ///     .set_taxman_code(
    ///         code,
    ///         |fee_denom, fee_rate| bolt_mock_taxman::InstantiateMsg {
    ///             config: bolt_mock_taxman::Config { fee_denom, fee_rate },
    ///         },
    ///     )
    ///     .build();
    /// ```
    ...
    AppError: From<VM::Error>,
{
    ...
    /// };
    ///
    /// let code = ContractBuilder::new(Box::new(bolt_mock_account::instantiate))
    ...
    ///     .set_account_code(
    ///         code,
    ///         |public_key| bolt_mock_account::InstantiateMsg { public_key },
    ///     )
    ///     .add_account("owner", Coins::new())
    ///     .set_owner("owner")
    ///     .build();
    ...
    where
        T: Into<Binary>,
        F: Fn(bolt_mock_account::PublicKey) -> M1A + 'static,
    {
        TestBuilder {
            db: self.db,
            vm: self.vm,
