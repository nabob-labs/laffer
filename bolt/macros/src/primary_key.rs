...
pub fn process(input: TokenStream) -> TokenStream {
    ...
    let raw_keys_arms = variant_idents.iter().zip(&indices).map(|(ident, idx)| {
        quote! {
            #name::#ident => ::std::vec![::bolt::RawKey::Fixed8([#idx])],
        }
    });
    ...
    quote! {
        impl ::bolt::PrimaryKey for #name {
            ...
            fn raw_keys(&self) -> ::std::vec::Vec<::bolt::RawKey<'_>> {
                match self {
                    #(#raw_keys_arms)*
                }
            }

            fn from_slice(bytes: &[u8]) -> ::bolt::StdResult<Self::Output> {
                match bytes {
                    #(#from_slice_arms)*
                    _ => ::std::result::Result::Err(::bolt::StdError::deserialize::<
                        Self::Output,
                        _,
                        ::bolt::Binary,
                    >(
