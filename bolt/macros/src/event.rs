...
pub fn process(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    quote::quote! {
        ...
        impl ::bolt::EventName for #input_name {
            const EVENT_NAME: &'static str = #name;
        }

        impl TryFrom<#input_name> for ::bolt::ContractEvent {
            type Error = ::bolt::StdError;

            fn try_from(event: #input_name) -> ::bolt::StdResult<Self> {
                Self::new(&event)
            }
        }
    }
