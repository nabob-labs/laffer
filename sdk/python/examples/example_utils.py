"""Setup helpers for the native Velox Python SDK examples.

Two factories:

* :func:`setup` — full ``(address, info, exchange)`` trio. Loads ``.env``
  for ``VELOX_SECRET_KEY`` / ``VELOX_ACCOUNT_ADDRESS`` and runs an equity
...
* ``VELOX_SECRET_KEY`` — raw hex secret. Required.
* ``VELOX_ACCOUNT_ADDRESS`` — required for HL-compat (Velox decouples key
...
if TYPE_CHECKING:
    from velox.exchange import Exchange
...
def setup(
    ...
    *,
    ...
    default (mainnet) — Velox has no canonical URL → contract mapping
    and we don't try to guess.
    """

    # `perp_dexs` is accepted for HL-signature symmetry but unused on the
    # native side — Velox has no builder-deployed DEX abstraction.
    _ = perp_dexs
    ...
    if state is None or float(state["margin"]) == 0:
        ...
        raise Exception(
            ...
            f"If the address shown is your API wallet address, set VELOX_ACCOUNT_ADDRESS "
...
def setup_read_only(
    ...
    *,
    ...
    # be forced to maintain VELOX_* secrets just to query public chain state.
    from velox.info import Info
    from velox.utils.constants import LOCAL_API_URL
...
def get_secret_key() -> str:
    """Resolve the signer's secret from ``VELOX_SECRET_KEY``."""

    secret_key = os.environ.get("VELOX_SECRET_KEY", "").strip()

    if not secret_key:
        raise ValueError(
            "VELOX_SECRET_KEY is required — set it in examples/.env or your environment.",
        )
...
def resolve_account_address(account: LocalAccount) -> str:
    """Return ``VELOX_ACCOUNT_ADDRESS`` or fall back to the wallet's address.

    Velox's HL-compat ``Exchange`` constructor REQUIRES an explicit
    ``account_address`` — we don't silently default to the wallet's derived
    address because the Velox-side address depends on the KeyType
    ...
    address = os.environ.get("VELOX_ACCOUNT_ADDRESS", "").strip()
    if not address:
