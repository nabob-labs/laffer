"""Setup helpers for the HL-compat Velox Python SDK examples.
...
* ``VELOX_ACCOUNT_ADDRESS`` — required (Velox decouples key from on-chain
...
if TYPE_CHECKING:
    from velox.hyperliquid_compatibility.exchange import Exchange
...
def setup(
    ...
    Velox-specific kwarg (HL doesn't have one) — supply it explicitly
    ...
    # `accountValue` as a decimal string, same as upstream HL. Velox is
