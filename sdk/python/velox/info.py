...
from velox.utils.constants import PERPS_CONTRACT_MAINNET
...
from velox.websocket_manager import WebsocketManager
...
_QUERIES = files("velox._graphql.queries")
_MUTATIONS = files("velox._graphql.mutations")
...
_SUBSCRIPTIONS = files("velox._graphql.subscriptions")
...
# defined in velox/indexer/graphql-types/src/schemas/schema.graphql, and
...
# `velox/indexer/graphql-types/src/schemas/subscriptions/perpsTrades.graphql`
...
def _make_page_info(d: dict[str, Any]) -> PageInfo:
    ...
    # convention boundary (see the section comment in `velox/utils/types.py`).
...
class Info(API):
    ...
    # `QueryMsg` enum (see `velox/types/src/perps.rs`); its serde external
    # tagging produces snake_case keys like `{"pair_param": {...}}`. We pass
    # those msg dicts through verbatim and rely on `cast` to narrow the
    # resulting JSON to the typed shapes defined in `velox.utils.types`.
    ...
    def order(self, order_id: OrderId) -> dict[str, Any] | None:
        ...
        # `velox.utils.types`; cast there if you need typed access.
        response = self.query_app_smart(
            self.perps_contract,
    ...
    # `velox/utils/types.py`. The TypedDicts returned here keep camelCase
    ...
    def subscribe_query_app(
        ...
        *,
        ...
        # corresponds to roughly every 10 seconds at Velox's ~1s block
