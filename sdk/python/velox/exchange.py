...
from velox.api import API
from velox.info import Info
from velox.utils.constants import (
    ACCOUNT_FACTORY_CONTRACT,
    GAS_OVERHEAD_SECP256K1,
    PERPS_CONTRACT_MAINNET,
    SETTLEMENT_DENOM,
)
from velox.utils.signing import Secp256k1Wallet, SingleSigner, Wallet
...
class Exchange(API):
    """Build, sign, and broadcast Velox transactions on behalf of one account."""
    ...
    # the empirically-measured cost for a Secp256k1 verify on Velox;
    ...
    def __init__(
        ...
        *,
        ...
        # signed with raw secp256k1 over SHA-256 (Velox's KeyType=1
    ...
    def address(self) -> Addr:
        """The Velox account address this Exchange transacts as."""
    ...
    def deposit_margin(self, amount: int) -> dict[str, Any]:
        ...
        #
        # Wire shape (Rust source: `velox/types/src/perps.rs::TraderMsg::Deposit`):
        #
        ...
        #   * Enum keys are snake_case because `#[bolt::derive(Serde)]`
        #     injects `rename_all = "snake_case"` (see
        #     `bolt/macros/src/derive.rs`).
    ...
    def withdraw_margin(self, amount: float | str | Decimal) -> dict[str, Any]:
        ...
        # uses `UsdValue`/6-decimal). `velox_decimal()` formats the
        # latter; the deposit path str()s the int directly.
        message: Message = {
            "execute": {
                "contract": self._perps_contract,
                "msg": {"trade": {"withdraw": {"amount": velox_decimal(amount)}}},
                "funds": {},
            },
        }
    ...
    def _build_submit_order_wire(
        ...
        *,
        ...
        # client-side for a friendlier error). `velox_decimal` already
        # rejects NaN/Inf inputs, so we don't pre-check those here.
        size_str = velox_decimal(size)

        # `velox_decimal` always pads to 6 dp, so a zero check on the
        ...
        # we also rely on bolt-derived enums serializing variant tags
        # as snake_case (`market`/`limit`) — `rename_all = "snake_case"`
        # is injected by `bolt/macros/src/derive.rs:93`.
    ...
    def _wrap_perps_execute(self, key: str, inner: dict[str, Any]) -> Message:
        ...
        # `velox/types/src/perps.rs::ExecuteMsg`) — so the dispatch key is
    ...
    def submit_market_order(
        ...
        *,
        ...
        kind = cast(
            "OrderKind",
            {"market": {"max_slippage": velox_decimal(max_slippage)}},
        )
    ...
    def submit_limit_order(
        ...
        *,
        ...
        limit_payload: dict[str, Any] = {
            "limit_price": velox_decimal(limit_price),
    ...
    def submit_conditional_order(
        ...
        if size is None:
            ...
            # `velox_decimal` collapses every zero-equivalent input to
        ...
        inner: dict[str, Any] = {
            "submit_conditional_order": {
                "pair_id": pair_id,
                "size": size_str,
                "trigger_price": velox_decimal(trigger_price),
                "trigger_direction": trigger_direction.value,
                "max_slippage": velox_decimal(max_slippage),
            },
        }
    ...
    def add_liquidity(
        ...
        *,
        ...
        # Wire shape (Rust source: `velox/types/src/perps.rs::VaultMsg::AddLiquidity`):
        #
        #   * `amount` is a `UsdValue` — 6-decimal fixed-point string,
        #     e.g. "1000.000000". `velox_decimal` produces this canonical
        ...
        # error.
        amount_str = velox_decimal(amount)
    ...
    def remove_liquidity(self, shares_to_burn: int) -> dict[str, Any]:
        ...
        # Wire shape (Rust source: `velox/types/src/perps.rs::VaultMsg::RemoveLiquidity`):
        #
    ...
    def set_referral(self, referrer: int | str) -> dict[str, Any]:
        ...
        # Wire shape (Rust source: `velox/types/src/perps.rs::ReferralMsg::SetReferral`):
        #
        ...
        if isinstance(referrer, int):
            ...
            # `User` struct (`velox/types/src/account_factory/msg.rs`).
    ...
    def liquidate(self, user: Addr) -> dict[str, Any]:
        ...
        # Wire shape (Rust source: `velox/types/src/perps.rs::MaintainerMsg::Liquidate`):
        #
        #   * `user` is the target trader's account `Addr`. The contract
        #     handler at `velox/perps/src/maintain/liquidate.rs` does
