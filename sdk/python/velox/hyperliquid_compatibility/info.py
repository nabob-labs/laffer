"""HL-shaped ``Info`` wrapper that translates HL read calls to native Velox calls.
...
* HL camelCase wire shapes are preserved verbatim (``assetPositions``,
  ...
  the wire pass through :func:`velox_decimal_to_hl_str` so trailing
  zeros are stripped (HL never sends ``"1.230000"``).
* HL-only or Velox-gap methods raise :class:`NotImplementedError` with a
...
* Subscriptions dispatch by the ``type`` field on the
  :class:`~velox.hyperliquid_compatibility.types.Subscription` dict.
...
from velox.hyperliquid_compatibility.types import (
    ActiveAssetData,
    Fill,
    L2BookData,
    L2Level,
    ...
    Subscription,
    Trade,
    velox_decimal_to_hl_str,
)
from velox.utils.constants import SETTLEMENT_DECIMALS
from velox.utils.types import (
    Addr,
    CandleInterval,
    OrderId,
    PairId,
...
if TYPE_CHECKING:
    import velox.info as _native_info
    from velox.utils.types import (
        PairState,
        PerpsCandle,
        PerpsEvent,
...
# --- Interval mapping -------------------------------------------------------
#
# HL exposes the union of intervals supported by the major venues; Velox's
...
_HL_TO_VELOX_INTERVAL: dict[str, CandleInterval] = {
...
def _hl_interval_to_velox(interval: str) -> CandleInterval:
    """Translate an HL candle interval string into the Velox ``CandleInterval`` enum."""
    ...
    try:
        return _HL_TO_VELOX_INTERVAL[interval]
    except KeyError as exc:
        supported = ", ".join(sorted(_HL_TO_VELOX_INTERVAL))
        raise ValueError(
            f"unsupported candle interval {interval!r}; Velox supports only: {supported}"
        ) from exc
...
def _pair_id_to_coin(pair_id: str) -> str:
    ...
    ``"perp/btcusd"`` → ``"BTC"``. Velox pair IDs are lowercase and always
...
# --- Reshape helpers --------------------------------------------------------
#
# Each function below takes a native Velox wire-shaped dict and returns the
...
def _to_hl_str(value: str | None, *, default: str = "0") -> str:
    """``velox_decimal_to_hl_str`` with a None-passthrough default."""
    ...
    return velox_decimal_to_hl_str(value)
...
def _reshape_user_state_to_hl(
    ...
    HL splits margin info into `marginSummary` (always cross on Velox,
    matches `crossMarginSummary`) and `assetPositions` (one entry per
    open position). Velox stores everything together in
    ...
    for pair_id, position in state["positions"].items():
        ...
        # up by this position. Velox's `equity` is global (account-level
        ...
        try:
            ...
            position_value_str = velox_decimal_to_hl_str(f"{position_value:.6f}")
        ...
        # Velox doesn't expose per-position margin, so we report 0 and
        ...
        asset_positions.append(
            {
                ...
                "position": {
                    "coin": coin,
                    "szi": velox_decimal_to_hl_str(size),
                    "entryPx": velox_decimal_to_hl_str(entry_px),
                    # Synthetic: Velox is cross-only, no leverage knob.
                    # `value=1` matches HL's "1x" default for accounts
                    # without explicit leverage adjustments.
                    "leverage": {"type": "cross", "value": 1},
                    "liquidationPx": (
                        velox_decimal_to_hl_str(liquidation_px)
                    ...
                    # Velox — no separate `cumFunding` series. Report
    ...
    margin_summary = {
        "accountValue": velox_decimal_to_hl_str(equity),
        "totalMarginUsed": velox_decimal_to_hl_str(state["reserved_margin"]),
        "totalNtlPos": velox_decimal_to_hl_str(f"{total_ntl_pos:.6f}"),
        "totalRawUsd": velox_decimal_to_hl_str(margin),
    }

    return {
        "assetPositions": asset_positions,
        # Velox is cross-margin only: cross and total share the same
        # numbers. HL clients that branch on `crossMarginSummary` vs
        # `marginSummary` will see equal values; that's correct since
        # Velox has no isolated-margin partition.
        "crossMarginSummary": margin_summary,
        "marginSummary": margin_summary,
        "withdrawable": velox_decimal_to_hl_str(available_margin),
    }
...
def _reshape_order_to_hl(order: dict[str, Any], order_id: str) -> dict[str, Any]:
    ...
    # Velox stores `size` as a signed quantity; HL splits this into
    ...
    sz_abs = velox_decimal_to_hl_str(f"{abs(size_f):.6f}")

    # `oid` is HL's int field name; Velox's order_id is a string.
    ...
    return {
        "coin": _pair_id_to_coin(pair_id),
        "side": side,
        "limitPx": velox_decimal_to_hl_str(order["limit_price"]),
        "sz": sz_abs,
        "oid": order_id,
        # Velox stores `created_at` as a Timestamp string (ns); HL
...
def _timestamp_ns_to_ms(ts: str | None) -> int:
    """Convert a Velox ns Timestamp string to HL's ms int. ``None`` → 0."""

    # Velox's `Timestamp` wire shape is a stringified ns count (per
    # `Timestamp = NewType("Timestamp", str)`), e.g. "1700000000000000000".
    # HL uses ms ints. The `// 1_000_000` integer-division is exact
    # because Velox never emits a non-integer ns count.
    if ts is None:
        return 0
...
def _reshape_fill_to_hl(event: PerpsEvent) -> Fill:
    ...
    # (trade-id and fee-token-symbol) which Velox doesn't model;
    # we synthesize stable values: `tid` is derived from
    # `(blockHeight, idx)` so it's unique per event, and `feeToken`
    # is hardcoded to "USDC" since Velox fees are settled in USDC
    ...
    # HL `side`: "A" = ask/sell, "B" = bid/buy. Velox's wire convention
    ...
    # `tid` is HL's per-fill trade id. Velox's `fillId` is a string;
    ...
    # `hash` is HL's tx-hash field. Velox's PerpsEvent has `txHash`.
    ...
    return cast(
        ...
        {
            "coin": coin,
            "px": velox_decimal_to_hl_str(fill_price),
            "sz": velox_decimal_to_hl_str(f"{abs(size_f):.6f}"),
            "side": side,
...
def _isotime_to_ms(iso: str | None) -> int:
    ...
    # Velox's indexer emits `createdAt` as ISO-8601, e.g.
...
def _reshape_l2_to_hl(
    ...
    ``{px, sz, n}``. Velox: ``{bids: {price: {size, notional}}, asks: ...}``.

    The ``n`` field (number of orders at the level) is not exposed by
    Velox's depth query — depth is bucketed and the bucket may aggregate
    ...
    def _level_list(side: dict[str, dict[str, str]], reverse: bool) -> list[L2Level]:
        ...
        return [
            L2Level(
                px=velox_decimal_to_hl_str(price),
                sz=velox_decimal_to_hl_str(side[price]["size"]),
                # Velox doesn't expose per-bucket order count — use 1
...
def _reshape_candle_to_hl(candle: PerpsCandle) -> dict[str, Any]:
    ...
    (trade count). Velox's candle has the same fields under longer
    ...
    return {
        # Despite the "Unix" suffix, Velox's `timeStartUnix`/`timeEndUnix`
        ...
        "i": candle["interval"],
        "o": velox_decimal_to_hl_str(candle["open"]),
        "c": velox_decimal_to_hl_str(candle["close"]),
        "h": velox_decimal_to_hl_str(candle["high"]),
        "l": velox_decimal_to_hl_str(candle["low"]),
        "v": velox_decimal_to_hl_str(candle["volume"]),
        # Velox doesn't track per-candle trade count separately;
...
def _reshape_pair_state_to_perp_ctx(
    ...
    # `funding`: Velox exposes `funding_rate` as a per-period rate.
    ...
    if pair_state is not None:
        funding = velox_decimal_to_hl_str(pair_state.get("funding_rate", "0"))
        # `openInterest` on HL is single-sided. Velox tracks long_oi
        ...
        open_interest = velox_decimal_to_hl_str(pair_state.get("long_oi", "0"))
    ...
    # Velox doesn't separately track day-base-volume (volume in the
    ...
    try:
        if current_price not in ("0", ""):
            day_base_vlm = velox_decimal_to_hl_str(
    ...
    # mid = book mid). On Velox, the contract's "oracle price" feeds
...
class Info:
    """HL-shaped facade over the native :class:`velox.info.Info`.
    ...
    def __init__(
        ...
        # `data not found! type: bolt_types::app::ContractInfo` against
        # any other chain.
    ...
    def _build_coin_resolver(self, *, meta: Meta | None = None) -> None:
        """Populate the four coin/asset/pair maps used for HL ↔ Velox translation."""
        ...
        if meta is not None:
            ...
            # `szDecimals`. We round-trip back to a Velox pair_id by
            # `_coin_to_pair_id`.
    ...
    def name_to_pair(self, name: str) -> PairId:
        """Translate an HL coin name to a Velox pair_id; raise ``KeyError`` if unknown."""
    ...
    def user_state(self, address: str, dex: str = "") -> dict[str, Any]:
        ...
        # `dex` is unused on Velox (no builder-deployed DEXes); we
    ...
    def open_orders(self, address: str, dex: str = "") -> list[dict[str, Any]]:
        """HL ``openOrders`` — flatten Velox's keyed map to a list."""

        _ = dex
        orders = self._native.orders_by_user(Addr(address))

        # Velox returns `dict[OrderId, item]`; HL returns a list. We
    ...
    def all_mids(self, dex: str = "") -> dict[str, str]:
        ...
        # `currentPrice` doubles as the mid price on Velox: it's the
    ...
    def meta(self, dex: str = "") -> Meta:
        ...
        # Velox doesn't have an `szDecimals` concept — we use the
    ...
    def meta_and_asset_ctxs(self) -> list[Any]:
        ...
        # HL atomically combines meta + ctx in one call. On Velox we
    ...
    def l2_snapshot(self, name: str) -> L2BookData:
        ...
        # Velox requires picking a `bucket_size` from the pair's
        ...
        # Velox doesn't include a server-side timestamp on the depth
    ...
    def candles_snapshot(
        ...
        pair_id = self.name_to_pair(name)
        velox_interval = _hl_interval_to_velox(interval)

        # HL takes ms timestamps; Velox's indexer's `laterThan` /
    ...
    def user_fills(self, address: str) -> list[Fill]:
        ...
        # Velox pushes both maker and taker rows for the same fill
    ...
    def query_order_by_oid(self, user: str, oid: int | str) -> dict[str, Any]:
        ...
        Velox's order storage is keyed by oid alone, so the ``user``
    ...
    def historical_orders(self, user: str) -> list[dict[str, Any]]:
        ...
        #   statusTimestamp: <ms>}`.
        # Velox doesn't fold the lifecycle on the server side; we
        ...
        for ev in persisted:
            ...
            order_dict = {
                "coin": _pair_id_to_coin(pair_id),
                "side": "B" if size_f > 0 else "A",
                "limitPx": velox_decimal_to_hl_str(limit_price),
                "sz": sz_abs,
                "oid": oid,
            ...
            if removed_event is not None:
                # The Velox `reason` enum aligns with HL: "filled",
    ...
    def spot_user_state(self, address: str) -> Any:
        raise NotImplementedError("Velox is perps-only")
    ...
    def user_staking_summary(self, address: str) -> Any:
        raise NotImplementedError("Velox has no HYPE-equivalent staking")
    ...
    def query_user_to_multi_sig_signers(self, multi_sig_user: str) -> Any:
        raise NotImplementedError("Velox multi-sig is not exposed via the perps SDK")

    def query_perp_deploy_auction_status(self) -> Any:
        raise NotImplementedError("Velox has no permissionless asset listing")

    def query_user_dex_abstraction_state(self, user: str) -> Any:
        raise NotImplementedError("HL-specific concept; no Velox analog")

    def query_user_abstraction_state(self, user: str) -> Any:
        raise NotImplementedError("HL-specific concept; no Velox analog")

    def user_twap_slice_fills(self, user: str) -> Any:
        raise NotImplementedError("Velox has no TWAP orders")

    def portfolio(self, user: str) -> Any:
        raise NotImplementedError("Velox has no per-user portfolio time-series")

    def user_role(self, user: str) -> Any:
        raise NotImplementedError("Velox has no role system")

    def user_rate_limit(self, user: str) -> Any:
        raise NotImplementedError("Velox has no per-user rate limit")

    def extra_agents(self, user: str) -> Any:
        raise NotImplementedError("Velox uses session credentials; no extraAgents analog")

    def funding_history(self, name: str, start_time: int, end_time: int | None = None) -> Any:
        raise NotImplementedError(
            "Velox folds funding into realized PnL on each fill; no separate funding stream"
        )
    ...
    def query_order_by_cloid(self, user: str, cloid: Any) -> Any:
        raise NotImplementedError("Velox stores cloid only at submit/cancel; no by-cloid lookup")
    ...
    def subscribe(
        ...
        if sub_type == "webData2":
            raise NotImplementedError("UI-specific aggregation; no Velox analog")
    ...
    def _subscribe_trades(
        ...
        # Velox pushes both maker and taker fills (paired by fill_id).
        ...
        def wrapped(trade: Any) -> None:
            ...
            sz_str = velox_decimal_to_hl_str(f"{abs(size_f):.6f}")
            ...
            hl_trade = Trade(
                coin=coin,
                side="B" if size_f > 0 else "A",
                px=velox_decimal_to_hl_str(trade.get("fillPrice", "0")),
                sz=cast("int", sz_str),
                hash="",
    ...
    def _subscribe_candle(
        ...
        return self._native.subscribe_perps_candles(pair_id, velox_interval, wrapped)
    ...
    def _subscribe_user_fills(
        ...
        # (a backfill of recent fills) and `False` thereafter. Velox's
    ...
    def _subscribe_order_updates(
        ...
        # statusTimestamp}` per message. Velox pushes one event per
    ...
    def _subscribe_active_asset_data(
        ...
        def wrapped(payload: Any) -> None:
            ...
            data = ActiveAssetData(
                ...
                # Velox the available margin is direction-agnostic
...
def _dedupe_fills(events: list[PerpsEvent]) -> list[Fill]:
    ...
    # Velox writes both the maker's and the taker's view of every
