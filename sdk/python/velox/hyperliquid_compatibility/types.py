...
import them by name with no source-level changes:
    ...
    from velox.hyperliquid_compatibility.types import OrderRequest, Cloid
...
class Cloid:
    """HL-compatible 16-byte hex client-order id; hashes to Uint64 for Velox.

    Round-trip warning: the cloid in Velox responses is the Uint64
    ...
    def to_uint64(self) -> int:
        """SHA-256-derived ``Uint64`` the Velox contract uses as ``ClientOrderId``.
        ...
        HL cloid carries 128 bits of entropy and a Velox
...
def velox_decimal_to_hl_str(x: str) -> str:
    """Strip trailing zeros from a Velox decimal string for HL wire shape.

    Velox canonicalises numbers as fixed-decimal strings (e.g. ``"1.230000"``)
    via ``velox_decimal``. HL's wire shape strips trailing zeros and drops
...
def hl_status_envelope(
    *,
    ...
    """Wrap a Velox outcome into HL's status envelope.
