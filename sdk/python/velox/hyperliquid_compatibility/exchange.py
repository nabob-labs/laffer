"""HL-shaped ``Exchange`` wrapper that translates HL-style signed actions to native Velox calls.
...
* HL camelCase wire shapes are preserved verbatim on the *response* envelopes
  ...
  wire pass through :func:`velox_decimal_to_hl_str` so trailing zeros are
  stripped.
...
* Methods that produce a tx return HL's status envelope:
  ...
  built with the Phase 15 helpers in :mod:`velox.hyperliquid_compatibility.types`.

Cloid asymmetry warning
-----------------------

HL's ``Cloid`` is 16 bytes; Velox's ``ClientOrderId`` is a Uint64. We hash
the cloid down via ``Cloid.to_uint64()`` (deterministic SHA-256 prefix). So
the cloid you see in a Velox response (extracted from indexer events) is
*not* the original 16-byte cloid you sent — see the module docstring of
``velox.hyperliquid_compatibility.types`` for the full rationale. Callers who
...
from velox.exchange import Exchange as NativeExchange
...
if TYPE_CHECKING:
    ...
    from velox.hyperliquid_compatibility.info import Info as HlInfo
    from velox.utils.signing import Wallet


# --- Internal helpers -------------------------------------------------------


# HL "Alo" (add-liquidity-only / post-only) maps to Velox's POST time-in-
# force. The Velox chain rejects an Alo limit order that would cross the
# book at submission, matching HL semantics. GTC and IOC are 1:1.
_HL_TIF_TO_VELOX: Final[dict[str, TimeInForce]] = {
    "Gtc": TimeInForce.GTC,
    "Ioc": TimeInForce.IOC,
...
def _hl_tif_to_velox(tif: str) -> TimeInForce:
    """Translate HL's TIF string ("Gtc"/"Ioc"/"Alo") to the Velox enum."""
    ...
    try:
        return _HL_TIF_TO_VELOX[tif]
    except KeyError as exc:
        supported = ", ".join(sorted(_HL_TIF_TO_VELOX))
        raise ValueError(
            f"unsupported HL time_in_force {tif!r}; supported: {supported}",
        ) from exc


def _hl_order_type_to_velox_kind(
    order_type: OrderType,
    limit_px: float,
    cloid: Cloid | None,
) -> OrderKind:
    """Translate an HL ``OrderType`` dict into a native Velox ``OrderKind``.
    ...
    if "limit" in order_type:
        ...
        tif = _hl_tif_to_velox(limit["tif"])
        ...
        return cast(
            ...
            {
                "limit": {
                    "limit_price": velox_decimal(limit_px),
    ...
    if "trigger" in order_type:
        ...
        # together with a trigger condition. Native Velox splits this into
...
def _signed_size(is_buy: bool, sz: float) -> float:
    """Convert HL's (is_buy, positive sz) to a Velox signed size (+ buy / − sell)."""

    # HL exposes side as a separate `is_buy` flag with a positive `sz`.
    # Velox encodes side in the sign of `size`. The translation is just
...
def _native_outcome_to_resting_envelope(
    ...
    *,
    ...
    Velox broadcast outcome to recover ``OrderFilled`` / ``OrderPersisted``
...
def _extract_error_message(outcome: dict[str, Any]) -> str | None:
    ...
    Velox's broadcast outcome can carry an error in several places depending
    ...
    # `result.err` is the GenericResult variant Velox emits when the tx
...
class Exchange:
    """HL-shaped facade over the native :class:`velox.exchange.Exchange`.
    ...
    def __init__(
        ...
        # Velox has no vault-account abstraction at the wallet level (vault
        ...
        if vault_address is not None:
            raise NotImplementedError(
                "vault_address is HL-specific — Velox exposes vault liquidity "
        ...
        if account_address is None:
            raise ValueError(
                "account_address is required for the Velox HL-compat Exchange; "
                "Velox decouples the signing key from the account address",
            )

        # `base_url` defaults to the local URL when None — HL's default is
        # the production mainnet, but Velox has no canonical default and we
        ...
        from velox.hyperliquid_compatibility.info import Info
    ...
    def order(
        ...
        # where third-party UIs can take a cut. Velox has no analog —
        # raise rather than silently drop the parameter and let HL traders
        # think they're paying their UI builder.
        if builder is not None:
            raise NotImplementedError("Velox has no builder fee marketplace")
    ...
    def bulk_orders(
        ...
        *,
        ...
        for req in request_list:
            ...
            cloid = req.get("cloid")
            order_kind = _hl_order_type_to_velox_kind(
                req["order_type"],
                req["limit_px"],
                cloid,
            )
    ...
    def modify_order(
        ...
        Per the API design notes, Velox has no first-class "modify" message —
    ...
    def market_open(
        ...
        *,
        ...
        # Velox's market order computes its own slippage band internally
        # against the contract's mark price, so HL's `px` is redundant
        # (and potentially wrong if HL's mid drifted from Velox's mark).
        # Document the divergence rather than pretend we use it.
        _ = px
        if builder is not None:
            raise NotImplementedError("Velox has no builder fee marketplace")

        # `cloid` doesn't flow into a native market order: Velox's
        ...
        if cloid is not None:
            raise NotImplementedError(
                ...
                "Velox's submit_market_order does not accept a client_order_id; "
    ...
    def market_close(
        ...
        *,
        ...
        _ = px  # See market_open — Velox ignores HL's px hint.
    ...
    #
    # The methods below either have no Velox analog (hard gaps) or were
    ...
    def update_leverage(
        ...
        raise NotImplementedError(
            "Velox is cross-margin only; per-asset leverage cannot be set. "
    ...
    def update_isolated_margin(self, amount: float, name: str) -> Any:
        raise NotImplementedError("Velox has no isolated margin")

    def schedule_cancel(self, time: int | None) -> Any:
        raise NotImplementedError("Velox has no scheduled cancellation")

    def usd_class_transfer(self, amount: float, to_perp: bool) -> Any:
        raise NotImplementedError("Velox is perps-only; no spot/perp split")
    ...
    def sub_account_transfer(
        ...
        raise NotImplementedError(
            "deferred — needs Bolt bank-message wrapping",
        )
    ...
    def sub_account_spot_transfer(
        ...
        raise NotImplementedError("Velox is perps-only; no spot transfers")
    ...
    def approve_agent(self, name: str | None = None) -> Any:
        raise NotImplementedError(
            "deferred — Velox session credentials need separate API",
        )
    ...
    def spot_deploy_register_token(
        ...
        raise NotImplementedError("Velox is perps-only; no spot deploys")
    ...
    def perp_deploy_register_asset(
        ...
        raise NotImplementedError("Velox has no permissionless perp deploys")
...
# Re-export the `velox_decimal_to_hl_str` symbol so callers that follow
...
# `Info` module re-exports `velox_decimal_to_hl_str`.
__all__ = [
    "Exchange",
    "velox_decimal_to_hl_str",
]
