...
from velox.utils.constants import ACCOUNT_FACTORY_CONTRACT
...
def sign_doc_canonical_json(sign_doc: SignDoc) -> bytes:
    ...
    # Mirrors `bolt::SignData::to_prehash_sign_data`, which calls
    ...
    # `#[bolt::derive(Serde)]` in bolt/macros/src/derive.rs). When the
...
# Ethereum's BIP-44 coin type. Velox's Secp256k1 wallet uses the same default
# so that an Ethereum mnemonic and a Velox Secp256k1 key derived from the same
# words land on the same private key (not the same Velox address — see
...
class Secp256k1Wallet:
    """Holds a 32-byte secp256k1 secret plus the Velox account address it controls."""
    ...
    def from_eth_account(
        ...
        """Re-use a LocalAccount's secp256k1 secret as a Velox Secp256k1 key."""

        # IMPORTANT: the Velox address derived from this key uses
        # key_tag=1 (Secp256k1), NOT key_tag=2 (Ethereum). It will therefore
        # differ from any Velox account previously activated through an
        # EIP-712 path with the same Ethereum key. Caller must pass the
        # correct Velox address explicitly; this adapter does not derive it.
        return cls(bytes(account.key), address)

    @property
    def address(self) -> Addr:
        """The Velox account address supplied at construction."""
    ...
    def key(self) -> Key:
        ...
        # `ByteArray<33>` in bolt serializes via Base64Encoder (see
        # `bolt/types/src/binary.rs`), not hex. Cast through Key (a TypedDict
        ...
        # is lowercase because `#[bolt::derive(Serde)]` injects
        # `rename_all = "snake_case"` (bolt/macros/src/derive.rs).
    ...
    def key_hash(self) -> Hash256:
        ...
        # Hash256's wire form is uppercase hex (Hash256 standard in bolt);
    ...
    def sign(self, sign_doc: SignDoc) -> Signature:
        ...
        # Velox's `Signature::Secp256k1` stores the 64-byte non-recoverable
        ...
        # Variant tag is lowercase per `#[bolt::derive(Serde)]`'s injected
        # `rename_all = "snake_case"` (bolt/macros/src/derive.rs).
...
# Phase 6 hasn't shipped yet, so we can't import `velox.info.Info` directly
...
class SingleSigner:
    ...
    def __init__(
        ...
        *,
        ...
        # that the same key can sign for multiple Velox accounts. The Wallet
    ...
    def query_user_index(self, info: _QueryClient) -> int:
        ...
        # layer, and v1 only supports the canonical Velox deployment. If
        ...
        # `velox_account_factory::QueryMsg`, so the wire form is
    ...
    def sign_tx(self, messages: list[Message], chain_id: str, gas_limit: int) -> Tx:
        ...
        # `velox_types::auth::Credential`, hence the `{"standard": ...}`
        # outer wrapper (variant tag is snake_case per
        # `#[bolt::derive(Serde)]`'s injected `rename_all = "snake_case"`).
