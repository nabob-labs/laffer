"""Typed wire shapes and velox_decimal helper used across the SDK."""
...
def velox_decimal(x: float | int | str | Decimal, max_places: int = _DEFAULT_MAX_PLACES) -> str:
    ...
    if isinstance(x, bool) or not isinstance(x, int | float | str | Decimal):
        raise TypeError(f"unsupported type for velox_decimal: {type(x).__name__}")

    if isinstance(x, float):
        if x != x or x in (float("inf"), float("-inf")):
            raise ValueError(f"velox_decimal does not accept non-finite floats: {x!r}")
        d = Decimal(str(x))
    ...
    if not d.is_finite():
        raise ValueError(f"velox_decimal does not accept non-finite Decimals: {x!r}")
...
class KeyType(StrEnum):
    ...
    # variants below. See velox/types/src/auth/key.rs for the source mapping.
    SECP256R1 = "SECP256R1"
    SECP256K1 = "SECP256K1"
    ETHEREUM = "ETHEREUM"
...
class Trade(TypedDict):  # noqa: N815
    ...
    # (velox/indexer/graphql-types/src/schemas/schema.graphql) and API doc §8.2. Keys stay
