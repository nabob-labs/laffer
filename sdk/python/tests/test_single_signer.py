"""Tests for velox.utils.signing.SingleSigner — state, queries, build/sign Tx."""
...
from velox.utils.signing import Secp256k1Wallet, SingleSigner
from velox.utils.types import Addr, Metadata, Tx, UnsignedTx
...
# Responses are keyed by (contract_addr, top-level msg variant) since Velox's
...
class TestConstruction:
    ...
    def test_address_property(self) -> None:
        ...
        # can sign for multiple Velox accounts; the signer is bound to one.
        wallet = _wallet()
...
class TestBuildUnsignedTx:
    def test_returns_correct_wire_shape(self) -> None:
        ...
        tx = signer.build_unsigned_tx([], "velox-1")
    ...
    def test_metadata_carries_chain_state(self) -> None:
        ...
        signer = SingleSigner(_wallet(), _DEMO_ADDRESS, user_index=42, next_nonce=7)
        tx: UnsignedTx = signer.build_unsigned_tx([], "velox-1")
        metadata = tx["data"]
        assert metadata["user_index"] == 42
        assert metadata["chain_id"] == "velox-1"
    ...
    def test_raises_if_user_index_unresolved(self) -> None:
        ...
        with pytest.raises(RuntimeError, match="user_index unresolved"):
            signer.build_unsigned_tx([], "velox-1")
...
class TestSignTx:
    def test_returns_tx_with_credential(self) -> None:
        ...
        signer = SingleSigner(_wallet(), _DEMO_ADDRESS, user_index=1, next_nonce=0)
        tx: Tx = signer.sign_tx([], "velox-1", 100_000)
        keys = set(cast(dict[str, Any], tx).keys())
    ...
    def test_credential_is_standard_secp256k1(self) -> None:
        ...
        tx = signer.sign_tx([], "velox-1", 100_000)
    ...
    def test_increments_nonce_on_success(self) -> None:
        ...
        signer.sign_tx([], "velox-1", 100_000)
    ...
    def test_consecutive_signs_use_consecutive_nonces(self) -> None:
        ...
        signer = SingleSigner(_wallet(), _DEMO_ADDRESS, user_index=1, next_nonce=0)
        first = signer.sign_tx([], "velox-1", 100_000)
        second = signer.sign_tx([], "velox-1", 100_000)
