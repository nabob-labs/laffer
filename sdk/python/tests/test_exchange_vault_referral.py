"""Tests for velox.exchange.Exchange — vault, referrals, and liquidation methods (Phase 14)."""

from __future__ import annotations

import pytest

from velox.utils.constants import ACCOUNT_FACTORY_CONTRACT, PERPS_CONTRACT_MAINNET
...
class TestAddLiquidity:
    ...
    def test_amount_uses_six_decimal_string(self) -> None:
        """Float amounts go through `velox_decimal` and produce 6-decimal USD strings."""

        # UsdValue is a 6-decimal fixed-point string; the same
        # encoding used by `withdraw_margin`. Pin a non-integer
        # float to confirm we're going through `velox_decimal`
    ...
    def test_rejects_bool_amount(self) -> None:
        """Bool amount is rejected via velox_decimal's bool guard."""

        # `velox_decimal` already rejects bool (see types.py). This
    ...
    def test_rejects_non_finite_amount(self) -> None:
        """NaN/Inf amounts are rejected (delegated to velox_decimal)."""
...
class TestLiquidate:
    def test_full_wire_shape(self) -> None:
        ...
        # `velox/perps/src/maintain/liquidate.rs`); it's the chain's
