"""Tests for velox.hyperliquid_compatibility.exchange."""
...
from velox.hyperliquid_compatibility.exchange import (
    Exchange as HlExchange,
)
from velox.hyperliquid_compatibility.exchange import (
    _build_submit_action,
    _extract_error_message,
    _hl_order_type_to_velox_kind,
    _hl_tif_to_velox,
    _native_outcome_to_cancel_envelope,
    _native_outcome_to_resting_envelope,
    _signed_size,
)
from velox.hyperliquid_compatibility.types import Cloid, OrderType
...
# A minimal fake of the native Velox Exchange that captures every call.
# The HL-compat Exchange constructs a native `velox.exchange.Exchange`
...
def _make_exchange(
    *,
    ...
    # wrapper does `from velox.exchange import Exchange as NativeExchange`
    ...
    import velox.hyperliquid_compatibility.exchange as exchange_module
    import velox.hyperliquid_compatibility.info as info_module
...
class TestHlTifToVelox:
    ...
    def test_unknown_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="unsupported HL time_in_force"):
            _hl_tif_to_velox("FOK")


class TestHlOrderTypeToVeloxKind:
    def test_limit_gtc_no_cloid(self) -> None:
        ...
        kind = _hl_order_type_to_velox_kind(order_type, 60_000.0, None)
    ...
    def test_limit_with_cloid_hashes_to_uint64_string(self) -> None:
        ...
        kind = _hl_order_type_to_velox_kind(order_type, 50_000.0, cloid)
        assert kind == {
            "limit": {
    ...
    def test_alo_routes_to_post_tif(self) -> None:
        """An Alo HL order produces a Velox POST limit kind."""

        kind = _hl_order_type_to_velox_kind({"limit": {"tif": "Alo"}}, 1.0, None)
    ...
    def test_trigger_branch_raises(self) -> None:
        ...
        # comment in `_hl_order_type_to_velox_kind`. The error message
        ...
        with pytest.raises(NotImplementedError, match="HL trigger orders"):
            _hl_order_type_to_velox_kind(order_type, 0.0, None)
    ...
    def test_empty_order_type_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="must contain 'limit' or 'trigger'"):
            _hl_order_type_to_velox_kind({}, 1.0, None)
...
class TestBuildSubmitAction:
    def test_buy_action_signs_size_positive(self) -> None:
        ...
        from velox.utils.types import OrderKind as _OrderKind
...
class TestExchangeConstructor:
    ...
    def test_vault_address_raises(self) -> None:
        ...
        # HL's vault routing has no analog in Velox (vault liquidity is
...
class TestMarketOpen:
    ...
    def test_px_arg_is_ignored(self) -> None:
        """HL's px hint is ignored; Velox computes its own band."""
...
class TestNotImplementedStubs:
    ...
    def test_spot_transfer(self) -> None:
        """spot_transfer raises (Velox is perps-only)."""
    ...
    def test_token_delegate(self) -> None:
        """token_delegate raises (no Velox analog)."""

        ex, _native, _info = _make_exchange()
        with pytest.raises(NotImplementedError, match="no Velox analog"):
            ex.token_delegate("0xvalidator", 100, is_undelegate=False)

    def test_use_big_blocks(self) -> None:
        """use_big_blocks raises (no Velox analog)."""
    ...
    def test_gossip_priority_bid(self) -> None:
        """gossip_priority_bid raises (no Velox analog)."""
    ...
    def test_spot_deploy_methods(self) -> None:
        """All spot_deploy_* methods raise (Velox is perps-only)."""
