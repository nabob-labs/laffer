"""Tests for velox.hyperliquid_compatibility.info."""
...
from velox.hyperliquid_compatibility.info import (
    Info,
    _coin_to_pair_id,
    _dedupe_fills,
    _hl_interval_to_velox,
    _isotime_to_ms,
    _ms_to_iso_str,
    _pair_id_to_coin,
...
from velox.utils.types import Addr, CandleInterval, PairId
...
class _FakeNativeInfo:
    """Fake of `velox.info.Info` returning canned native-shaped responses."""
    ...
    def perps_candles(self, pair_id: str, interval: Any, **kwargs: Any) -> Any:
        from velox.utils.types import Connection, PageInfo
...
def _make_info(
    ...
    **kwargs: Any,
    ...
    with patch.object(velox.info, "Info", return_value=fake):
...
class TestIntervalMapping:
    def test_supported_intervals_resolve(self) -> None:
        """Every Velox-backed HL interval string maps to the right enum."""
        ...
        for hl_value, velox_value in cases.items():
            assert _hl_interval_to_velox(hl_value) == velox_value

    def test_unsupported_interval_raises(self) -> None:
        """HL intervals Velox doesn't support raise ValueError."""

        with pytest.raises(ValueError, match="unsupported candle interval '3m'"):
            _hl_interval_to_velox("3m")
...
class TestReshapeFill:
    def test_buy_fill_with_open(self) -> None:
        ...
        # `oid` on the Fill TypedDict is annotated `int`, but Velox
...
class TestReshapeL2:
    def test_levels_are_sorted_correctly(self) -> None:
        ...
        # Default `n=1` for every level (Velox doesn't expose count).
...
class TestInfoConstruction:
    def test_default_resolver_from_pair_params(self) -> None:
        ...
        # `name_to_coin` is identity for Velox (no vault aliases).
        assert info.name_to_coin == {"BTC": "BTC", "ETH": "ETH"}
        # All assets share the same szDecimals (Velox is uniform 6).
...
class TestCandlesSnapshot:
    def test_passes_interval_and_time_bounds(self) -> None:
        """`candles_snapshot` translates HL interval and ms times to Velox."""
    ...
    def test_unsupported_interval_raises(self) -> None:
        """An HL interval Velox doesn't have raises ValueError."""
...
class TestSubscribeNotImplemented:
    ...
    def test_raises_not_implemented(self, subscription: Any) -> None:
        """Subscriptions Velox can't serve raise NotImplementedError."""
