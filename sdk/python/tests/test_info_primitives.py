"""Tests for velox.info.Info — GraphQL query primitives."""
...
from velox.utils.signing import _QueryClient
from velox.utils.types import Addr, Metadata, Nonce, Tx, UnsignedTx, UserIndex
...
class TestQueryStatus:
    def test_returns_chain_id_and_block(self, httpserver: HTTPServer) -> None:
        ...
        _capture_request(
            httpserver,
            {"data": {"queryStatus": {"chainId": "velox-1", "block": block}}},
        )
        result = _info(httpserver).query_status()
        assert result == {"chainId": "velox-1", "block": block}
    ...
    def test_posts_query_status_document(self, httpserver: HTTPServer) -> None:
        ...
        captured = _capture_request(
            httpserver,
            {"data": {"queryStatus": {"chainId": "velox-1", "block": {}}}},
        )
