...
//!
//! For each generated query and subscription type, spin up a mock velox-httpd
...
//! `velox/testing/tests/httpd/`.

use {
    assertor::*,
    velox_genesis::GenesisOption,
    velox_mock_httpd::{BlockCreation, TestOption, run_with_port_sender, wait_for_server_ready},
    velox_sdk::{
...
/// Spawn a mock velox-httpd in a separate thread and return the bound port
...
subscription_smoke_test!(
    ...
    |data: subscribe_trades::ResponseData| {
        data.trades.base_denom == "velox" && data.trades.quote_denom == "bridge/usdc"
    },
    false
);
