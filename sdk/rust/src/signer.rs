...
/// Utility for signing transactions in the format by Velox's single-signature
/// accounts.
#[derive(Debug)]
    ...
    N: MaybeDefined<Nonce>,
{
        ...
        anyhow::Error: From<C::Error>,
    {
        let account_factory = client
            .query_app_config::<velox_types::config::AppConfig>(None)
            .await?
            .addresses
            .account_factory;
...
mod tests {
    use {
        ...
        velox_account_factory::USERS,
        velox_auth::{account::STATUS, authenticate_tx},
        velox_types::{
            account_factory::{User, Username},
            auth::AccountStatus,
            config::{AppAddresses, AppConfig},
        },
        bolt::{AuthMode, Coins, MockContext, MockQuerier, MockStorage, ResultExt, btree_map},
    };
