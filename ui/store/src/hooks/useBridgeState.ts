...
import type { HyperlaneConfig } from "@laffer/velox/types";
...
export function useBridgeState(params: UseBridgeStateParameters) {
  const { coins: allCoins, chain: veloxChain } = useConfig();
  ...
  const isTestnet = ["Devnet", "Testnet"].includes(veloxChain.name);
