...
import { Decimal, formatNumber } from "@laffer/velox/utils";

import { Cell, FormattedNumber, IconLink, Table, Tabs } from "@laffer/applets-kit";
...
import type { ConditionalOrder, OrderId, PairId } from "@laffer/velox/types";
  ...
  useEffect(() => {
    ...
    if (currentPrice) {
      ...
      const formatted = Number.isFinite(priceNum)
        ...
        : currentPrice;
      document.title = `${formatted} | ${symbol} | Velox`;
    } else {
      document.title = `${symbol} · Velox`;
    }
