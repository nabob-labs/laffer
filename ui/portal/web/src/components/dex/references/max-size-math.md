...
because Hyperliquid reserves per-side. Velox's chain reserves
...
position was opened at 1× there. Velox has no per-user per-pair
leverage; each pair contributes at its on-chain `initialMarginRatio`,
so the Velox number only coincides with Hyperliquid's when
...
- `velox/perps/src/core/decompose.rs` — decomposition into closing and
  opening portions.
- `velox/perps/src/core/margin.rs` (`check_margin`) — the pre-match
  margin check the UI formula mirrors.
- `velox/perps/src/trade/submit_order.rs` — reduce-only short-circuit
