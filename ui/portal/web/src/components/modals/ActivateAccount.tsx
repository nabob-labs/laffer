...
import { useAccount, useBalances, useConfig } from "@laffer/store";
...
export const ActivateAccount = forwardRef<unknown>((_props, _ref) => {
  ...
  const isMainnet = chain.id === "velox-1";
  const hasBalance = balances && Object.keys(balances).length > 0;

  const { mutate: claimFaucet, isPending } = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${window.velox.urls.faucetUrl}/${account!.address}?skip_check=true`);
