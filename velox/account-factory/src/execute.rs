use {
    ...
    anyhow::{bail, ensure},
    velox_auth::{VerifyData, verify_signature},
    velox_types::{
...
pub fn instantiate(ctx: MutableCtx, msg: InstantiateMsg) -> StdResult<Response> {
    // Save the code hashes associated with the Velox account contract.
...
fn onboard_new_user(
    ...
    Ok((
        Message::instantiate(
            ...
            salt,
            Some(format!("velox/account/single/{account_index}")),
            Some(factory),
...
fn register_account(ctx: MutableCtx) -> anyhow::Result<Response> {
    ...
    Ok(Response::new()
        .add_message(Message::instantiate(
            ...
            salt,
            Some(format!("velox/account/single/{index}")),
            Some(ctx.contract),
