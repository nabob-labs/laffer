...
#[cfg(feature = "metrics")]
use {bolt::NumberConst, std::collections::HashMap};
...
pub(crate) fn auction(ctx: MutableCtx) -> anyhow::Result<Response> {
    ...
    let app_cfg = ctx.querier.query_velox_config()?;
...
mod tests {
    use {
        ...
        bolt::{Bounded, MockContext, MockQuerier, NumberConst, Timestamp, Uint128},
        std::str::FromStr,
        test_case::test_case,
    };
    ...
    fn properly_determine_resting_order_book_state(
        ...
        let querier = MockQuerier::new()
            ...
            .with_raw_contract_storage(MOCK_ORACLE, |storage| {
                // Set the prices for velox and USDC.
        ...
        RESTING_ORDER_BOOK
            .load(&ctx.storage, (&velox::DENOM, &usdc::DENOM))
            .unwrap()
    }
}
