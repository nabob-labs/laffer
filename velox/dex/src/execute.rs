...
mod tests {
    use {
        ...
        bolt::{Addr, Bounded, MockContext, MockQuerier, NumberConst, Udec128},
    ...
    #[test_case(
        ...
        coins! {
            ...
            velox::DENOM.clone() => 600,
        },
        coins! {
            usdc::DENOM.clone() => 300,
            velox::DENOM.clone() => 400,
        };
        "overfunded both in one tx; send 600 usdc + 600 velox, require 300 usdc + 200 velox"
    )]
    fn overfunded_order_refund_works(
        ...
        let querier = MockQuerier::new().with_raw_contract_storage(dex_contract, |storage| {
            // Create the velox-usdc pair.
            // The specific parameters don't matter. We just need the pair to exist.
            PAIRS
                .save(storage, (&velox::DENOM, &usdc::DENOM), &PairParams {
