...
This however, doesn't work, due to limitations of the `velox_app::Vm` trait:

1. For the WASM VM, the `wasmer` runtime requires all elements in a functional environment to be `Send + 'static`.[^2]
2. The Velox app requires the storage instance to be `Clone`.
