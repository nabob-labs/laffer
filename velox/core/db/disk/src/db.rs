...
use uuid::Uuid;
#[cfg(feature = "metrics")]
use {crate::statistics, velox_primitives::MetricsIterExt};
use {
    ...
    velox_app::{CONTRACT_NAMESPACE, Commitment, Db, NAMESPACE_LEN},
    velox_primitives::{Addr, Batch, Buffer, Hash256, HashExt, Op, Order, Record, Storage},
};
    ...
    T: Commitment,
{
    ...
    fn flush_but_not_commit(&self, batch: Batch) -> DbResult<(u64, Option<Hash256>)> {
        ...
        // We assume Velox App has the following behavior: for each key that is
    ...
    fn commit(&self) -> DbResult<u64> {
        ...
        for (key, op) in pending.state_storage {
            let cf = if is_wasm_key(&key) { cf_wasm } else { cf_state };
...
mod tests_jmt {
    use {
        ...
        hex_literal::hex,
        velox_app::Db,
        velox_jmt::{MerkleTree, verify_proof},
        velox_primitives::{
    ...
    #[test]
    fn disk_db_works() {
        let path = TempDataDir::new("_velox_disk_db_works");
    ...
    fn disk_db_pruning_works() {
        let path = TempDataDir::new("_velox_disk_db_pruning_works");
        let db = DiskDb::<MerkleTree>::open(&path).unwrap();
        ...
        for version in 0..=2 {
            ...
            assert!(db.prove(b"a", Some(version)).is_err_and(|err| {
                err.to_string()
                    .contains("data not found! type: velox_jmt::node::Node")
            }));
        }
...
mod tests_simple {
    use {
        super::*,
        velox_app::{SimpleCommitment, StorageProvider},
        velox_primitives::{
    ...
    fn disk_db_lite_works() {
        let path = TempDataDir::new("_velox_disk_db_lite_works");
    ...
    #[test]
    fn priority_data() {
        let path = TempDataDir::new("_velox_disk_db_priority_data");
    ...
    fn priority_data_new_db() {
        let path = TempDataDir::new("_velox_disk_db_priority_data_new_db");
    ...
    fn rejecting_non_wasm_key() {
        ...
        let path = TempDataDir::new("_velox_disk_db_lite_iterator_works");
        let db = DiskDb::<SimpleCommitment>::open(&path).unwrap();
...
mod test_deadlock {
    use {
        super::*,
        std::time::Duration,
        velox_app::SimpleCommitment,
        velox_storage::{Index, IndexList, IndexedMap, MultiIndex},
        velox_temp_rocksdb::TempDataDir,
    };
    ...
    fn run_deadlock_test() {
        let path = TempDataDir::new("_velox_disk_db_deadlock_problem");
