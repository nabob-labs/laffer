use {
    ...
    tracing::info,
    velox_app::GAS_COSTS,
    velox_primitives::{Addr, BorshDeExt, BorshSerExt, Query, Record, Storage, decode_sections},
...
pub fn secp256k1_verify(
    ...
    match velox_crypto::secp256k1_verify(&msg_hash, &sig, &pk) {
        Ok(()) => Ok(0),
...
pub fn secp256r1_verify(
    ...
    match velox_crypto::secp256r1_verify(&msg_hash, &sig, &pk) {
        Ok(()) => Ok(0),
        Err(err) => Ok(err.into_error_code()),
    }
}
...
pub fn secp256k1_pubkey_recover(
    ...
    let (error_code, ptr) =
        match velox_crypto::secp256k1_pubkey_recover(&msg_hash, &sig, recovery_id, compressed) {
...
pub fn ed25519_verify(
    ...
    match velox_crypto::ed25519_verify(&msg_hash, &sig, &pk) {
        Ok(()) => Ok(0),
        Err(err) => Ok(err.into_error_code()),
    }
}
...
pub fn ed25519_batch_verify(
    ...
    match velox_crypto::ed25519_batch_verify(&prehash_msgs, &sigs, &pks) {
        Ok(()) => Ok(0),
        Err(err) => Ok(err.into_error_code()),
    }
}
...
macro_rules! impl_hash_method {
    ($hasher:ident, $name:literal) => {
        pub fn $hasher(mut fe: FunctionEnvMut<Environment>, data_ptr: u32) -> VmResult<u32> {
            ...
            let hash = velox_crypto::$hasher(&data);
...
mod tests {
    use {
        ...
        test_case::test_case,
        velox_app::{APP_CONFIG, GAS_COSTS, GasTracker, QuerierProviderImpl, StorageProvider},
        velox_identity::{Identity256, Identity512},
        velox_primitives::{
    ...
    const TESTER_CONTRACT: &[u8] = include_bytes!("../testdata/velox_tester.wasm");
    ...
    fn generate_secp256r1_verify_request() -> VerifyTest {
        ...
        let msg_hash = Identity256::from(velox_crypto::sha2_256(MSG));
        ...
        VerifyTest {
            ...
            wrong_msg: velox_crypto::sha2_256(WRONG_MSG).to_vec(),
        }
    }
    ...
    fn generate_ed25519_verify_request() -> VerifyTest {
        ...
        let vk = VerifyingKey::from(&sk);
        let msg_hash = Identity512::from(velox_crypto::sha2_512(MSG));
        ...
        VerifyTest {
            ...
            sig: sig.to_bytes().to_vec(),
            msg_hash: msg_hash.into_bytes().into(),
            wrong_msg: velox_crypto::sha2_512(WRONG_MSG).to_vec(),
        }
    }
    ...
    #[test_case(
        crate::sha2_256,
        velox_crypto::sha2_256;
        "sha2_256"
    )]
    #[test_case(
        crate::sha2_512,
        velox_crypto::sha2_512;
        "sha2_512"
    )]
    #[test_case(
        crate::sha2_512_truncated,
        velox_crypto::sha2_512_truncated;
        "sha2_512_truncated"
    )]
    #[test_case(
        crate::sha3_256,
        velox_crypto::sha3_256;
        "sha3_256"
    )]
    #[test_case(
        crate::sha3_512,
        velox_crypto::sha3_512;
        "sha3_512"
    )]
    #[test_case(
        crate::sha3_512_truncated,
        velox_crypto::sha3_512_truncated;
        "sha3_512_truncated"
    )]
    #[test_case(
        crate::keccak256,
        velox_crypto::keccak256;
        "keccak256"
    )]
    #[test_case(
        crate::blake2s_256,
        velox_crypto::blake2s_256;
        "blake2s_256"
    )]
    #[test_case(
        crate::blake2b_512,
        velox_crypto::blake2b_512;
        "blake2b_512"
    )]
    #[test_case(
        crate::blake3,
