...
pub fn process(input: TokenStream) -> TokenStream {
    ...
    let raw_keys_arms = variant_idents.iter().zip(&indices).map(|(ident, idx)| {
        quote! {
            #name::#ident => ::std::vec![::velox_storage::RawKey::Fixed8([#idx])],
        }
    });
    ...
    quote! {
        impl ::velox_storage::PrimaryKey for #name {
            type Output = Self;
            type Prefix = ();
            type Suffix = ();
            ...
            fn raw_keys(&self) -> ::std::vec::Vec<::velox_storage::RawKey<'_>> {
                match self {
                    #(#raw_keys_arms)*
                }
            }

            fn from_slice(bytes: &[u8]) -> ::velox_storage::__private::StdResult<Self::Output> {
                match bytes {
                    #(#from_slice_arms)*
                    _ => ::std::result::Result::Err(::velox_storage::__private::StdError::deserialize::<
                        Self::Output,
                        _,
                        ::velox_storage::__private::Binary,
                    >(
                        "key",
                        #error_msg,
                        bytes.into(),
