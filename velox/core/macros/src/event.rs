...
pub fn process(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    quote::quote! {
        ...
        impl ::velox_primitives::EventName for #input_name {
            const EVENT_NAME: &'static str = #name;
        }

        impl TryFrom<#input_name> for ::velox_primitives::ContractEvent {
            type Error = ::velox_primitives::StdError;

            fn try_from(event: #input_name) -> ::velox_primitives::StdResult<Self> {
                Self::new(&event)
            }
        }
    }
