use {
    velox_genesis::{Contracts, DexOption, GenesisOption},
    velox_taxman::VOLUMES_BY_USER,
    velox_testing::{
...
#[test_case(
    ...
    btree_map! {
        OrderId::new(!1) => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(9), // Receives one less due to fee
        ...
        OrderId::new(!3) => btree_map! {
            velox::DENOM.clone() => BalanceChange::Unchanged,
            usdc::DENOM.clone()  => BalanceChange::Decreased(100),
        },
        OrderId::new(4) => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(10),
...
#[test_case(
    ...
    btree_map! {
        ...
        OrderId::new(!2) => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(4),   // half filled, receives one less due to fee
        ...
        OrderId::new(!7) => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(4),
...
#[test_case(
    ...
    btree_map! {
        OrderId::new(!1) => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(19), // Receives one less due to fee
...
#[test_case(
    ...
    btree_map! {
        OrderId::new(!1) => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(24),
...
fn dex_works(
    ...
    // Register oracle price source for USDC and VELOX. Needed for volume tracking in cron_execute
    suite
        .execute(
            &mut accounts.owner,
            contracts.oracle,
    ...
    suite
        .execute(
            ...
            contracts.oracle,
            &oracle::ExecuteMsg::RegisterPriceSources(btree_map! {
                velox::DENOM.clone() => PriceSource::Fixed {
                    humanized_price: Udec128::ONE,
                    precision: 6,
                    timestamp: Timestamp::from_seconds(1730802926),
                },
    ...
    let txs = orders_to_submit
        ...
        .map(|((direction, price, amount_base), signer)| {
            ...
            let (amount, funds) = match direction {
                ...
                Direction::Ask => {
                    let funds = Coins::one(velox::DENOM.clone(), amount_base).unwrap();
                    (amount_base, funds)
                },
            };
...
fn submit_standard_order(
    ...
    let funds = match direction {
        ...
        Direction::Ask => Coins::one(velox::DENOM.clone(), 100_000_000).unwrap(),
    };
...
fn volume_tracking_works() {
    ...
    let mut user2_addr_2 = user2_addr_1
        .register_new_account(
            &mut suite,
            contracts.account_factory,
            Coins::one(velox::DENOM.clone(), 100_000_000).unwrap(),
        )
        .unwrap();
...
fn volume_tracking_works_with_multiple_orders_from_same_user() {
    ...
    // Submit two orders for VELOX/USDC and one for ETH/USDC with user1
    suite
        .execute(
            &mut accounts.user1,
            contracts.dex,
...
#[test_case(
    ...
    btree_map! {
        0 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(100_000_000),
            usdc::DENOM.clone() => BalanceChange::Unchanged,
        },
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Unchanged,
...
#[test_case(
    ...
    btree_map! {
        0 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(100_000_000),
            usdc::DENOM.clone() => BalanceChange::Decreased(100_000_000),
        },
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(100_000_000),
...
#[test_case(
    ...
    btree_map! {
        0 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(150_000_000),
            usdc::DENOM.clone() => BalanceChange::Increased(100_000_000),
        },
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(100_000_000),
...
#[test_case(
    ...
    btree_map! {
        ...
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(100_000_000), // partially filled limit order should stay on the book, half remaining
            usdc::DENOM.clone() => BalanceChange::Increased(52_500_000),
        },
        2 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(150_000_000),
...
#[test_case(
    ...
    btree_map! {
        ...
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(50_000_000),
            usdc::DENOM.clone() => BalanceChange::Decreased(50_000_000),
        },
        2 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(150_000_000),
...
#[test_case(
    ...
    btree_map! {
        ...
        2 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(30_000_000),
...
#[test_case(
    ...
    btree_map! {
        0 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(80_000_000),
            usdc::DENOM.clone() => BalanceChange::Decreased(100_000_000),
        },
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(50_000_000),
            usdc::DENOM.clone() => BalanceChange::Increased(50_000_000),
        },
        2 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(30_000_000),
...
#[test_case(
    ...
    btree_map! {
        ...
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(95_000_000),
            usdc::DENOM.clone() => BalanceChange::Decreased(100_000_000),
        },
    },
    BTreeMap::new();
...
#[test_case(
    ...
    btree_map! {
        ...
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(47_500_000),
            usdc::DENOM.clone() => BalanceChange::Decreased(100_000_000),
        },
    },
    BTreeMap::new();
...
#[test_case(
    ...
    btree_map! {
        0 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(99_000_000),
            usdc::DENOM.clone() => BalanceChange::Decreased(100_000_000),
        },
        1 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(100_000_000),
...
#[test_case(
    ...
    btree_map! {
        0 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(108_900_000),
        ...
        2 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(10_000_000),
...
#[test_case(
    ...
    btree_map! {
        0 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Decreased(110_000_000),
        ...
        2 => btree_map! {
            velox::DENOM.clone() => BalanceChange::Increased(9_500_000),
...
fn market_orders_are_sorted_by_price_ascending() {
    ...
    for i in 0..2 {
        ...
        suite.balances().should_change(first_user, btree_map! {
            ...
            velox::DENOM.clone() => BalanceChange::Unchanged, // no match
        });
        suite.balances().should_change(second_user, btree_map! {
            ...
            velox::DENOM.clone() => BalanceChange::Increased(1000000),
        });
    }
}
...
fn refund_left_over_market_bid() {
        ...
        setup_test_naive_with_custom_genesis(Default::default(), GenesisOption {
            dex: DexOption {
                pairs: vec![PairUpdate {
                    ...
                    quote_denom: usdc::DENOM.clone(),
                    params: PairParams {
                        lp_denom: Denom::from_str("dex/pool/velox/usdc").unwrap(),
    ...
    suite.balances().should_change(&accounts.user2, btree_map! {
        velox::DENOM.clone() => BalanceChange::Increased(1),
        usdc::DENOM.clone() => BalanceChange::Decreased(100),
    });
...
fn refund_left_over_market_ask() {
    ...
    suite.balances().should_change(&accounts.user2, btree_map! {
        velox::DENOM.clone() => BalanceChange::Decreased(1),
        usdc::DENOM.clone() => BalanceChange::Increased(100),
    });
