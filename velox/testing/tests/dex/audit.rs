...
use {
    velox_dex::liquidity_depth::get_bucket,
    velox_genesis::{DexOption, GenesisOption, OracleOption},
    velox_testing::{
...
fn liquidity_depth_from_passive_pool_decreased_properly_when_order_filled() {
    ...
    // Supply oracle prices. VELOX = $200, USDC = $1.
    suite
        .execute(
            &mut accounts.owner,
            contracts.oracle,
    ...
    // Configure VELOX-USD pool type to geometric with spacing 1. It's easier to
    // work with than xyk.
    suite
        .execute(
            &mut accounts.owner,
    ...
    // Add liquidity to the VELOX-USDC pool: 10 velox, 2,000 USDC (2_000_000_000 uusdc)
    suite
        .execute(
            &mut accounts.owner,
            contracts.dex,
    ...
    suite
        ...
        .should_succeed_and_equal(dex::LiquidityDepthResponse {
            bid_depth: Some(vec![(
                Price::new(199_400_000), // 200 * (1 - 0.3%), considering 6 decimal difference between velox and usdc
                LiquidityDepth {
...
fn issue_194_cancel_all_orders_works_properly_with_passive_orders() {
    ...
    // Set up VELOX-USDC pair with geometric pool and oracle feed.
    ...
    suite
        .query_balance(&contracts.dex, velox::DENOM.clone())
...
fn issue_296_provide_liquidity_query_uses_max_staleness_for_oracle_price() {
    ...
    suite
        .execute(
            ...
            contracts.oracle,
            &oracle::ExecuteMsg::RegisterPriceSources(btree_map! {
                velox::DENOM.clone() => oracle::PriceSource::Fixed {
                    humanized_price: Udec128::new(200),
                    precision: 0,
    ...
    suite
        .query_wasm_smart(contracts.dex, dex::QuerySimulateProvideLiquidityRequest {
            ...
            deposit: CoinPair::new_unchecked(
                ...
                Coin::new(velox::DENOM.clone(), 5).unwrap(),
            ),
        })
        .should_fail_with_error(
            ...
            //
            // USDC denom (`bridge/usdc`) < VELOX denom (`velox`), so the error
