use {
    ...
    std::{fmt::Debug, vec},
    test_case::test_case,
    velox_app::{AppError, NaiveProposalPreparer, NullIndexer},
    velox_backtrace::Backtraceable,
    velox_crypto::{sha2_256, sha2_512},
    velox_db_memory::MemDb,
    velox_genesis::{GenesisCodes, GenesisOption},
    ...
    velox_testing::{Preset, TestAccounts, TestSuite, setup_suite_with_db_and_vm},
    velox_vm_hybrid::HybridVm,
    velox_vm_rust::RustVm,
    velox_vm_wasm::VmError,
};
...
async fn setup_test() -> (
    ...
    let tester = suite
        .upload_and_instantiate_with_gas(
            &mut accounts.owner,
            320_000_000,
            read_wasm_file("velox_tester.wasm"),
            &velox_tester::InstantiateMsg {},
            "tester",
            Some("tester"),
            None,
            Coins::new(),
...
async fn infinite_loop() {
    ...
    suite
        .send_message_with_gas(
            ...
            Message::execute(
                tester,
                &velox_tester::ExecuteMsg::InfiniteLoop {},
                Coins::new(),
            )
            .unwrap(),
        )
...
async fn immutable_state() {
    ...
    suite
        .query_wasm_smart(
            tester,
            velox_tester::QueryForceWriteRequest {
                key: "larry".to_string(),
    ...
    suite
        .send_message_with_gas(
            ...
            Message::execute(
                tester,
                &velox_tester::ExecuteMsg::ForceWriteOnQuery {
                    key: "larry".to_string(),
                    value: "engineer".to_string(),
                },
                Coins::new(),
...
async fn query_stack_overflow() {
    ...
    suite
        .query_wasm_smart(tester, velox_tester::QueryStackOverflowRequest {})
...
async fn message_stack_overflow() {
    ...
    suite
        .send_message_with_gas(
            ...
            Message::execute(
                tester,
                &velox_tester::ExecuteMsg::StackOverflow {},
                Coins::new(),
            )
            .unwrap(),
        )
