use {
    velox_app::{NaiveProposalPreparer, NullIndexer},
    velox_backtrace::Backtraceable,
    velox_db_memory::MemDb,
    velox_genesis::{GenesisCodes, GenesisOption},
    velox_primitives::{Addr, Binary, Coins, HashExt, QuerierExt, Query, ResultExt},
    velox_tester::{
    ...
    velox_vm_rust::{ContractBuilder, RustVm},
};
...
pub async fn setup_test() -> (
    ...
    let rust_tester: Binary = ContractBuilder::new(Box::new(velox_tester::instantiate))
        .with_query(Box::new(velox_tester::query))
        .build()
        .into();
...
async fn do_backtrace_test() {
    ...
    assert!(!backtrace.contains("velox_tester::query::failing_query"));
