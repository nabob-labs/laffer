...
async fn index_candles_with_mocked_clickhouse() -> anyhow::Result<()> {
    ...
    let txs = orders_to_submit
        ...
        .map(|((direction, price, amount), signer)| {
            ...
            let fund = match direction {
                ...
                Direction::Ask => Coin::new(velox::DENOM.clone(), amount).unwrap(),
            };
    ...
    assert_that!(pair_price.base_denom).is_equal_to("velox".to_string());
...
async fn index_candles_with_real_clickhouse() -> anyhow::Result<()> {
    ...
    let pair_price_query_builder =
        PairPriceQueryBuilder::new("velox".to_string(), "bridge/usdc".to_string()).with_limit(1);
    ...
    assert_that!(candle_1m.base_denom).is_equal_to("velox".to_string());
...
async fn index_candles_with_real_clickhouse_and_one_minute_interval() -> anyhow::Result<()> {
    ...
    let pair_prices = PairPriceQueryBuilder::new("velox".to_string(), "bridge/usdc".to_string())
...
async fn index_candles_changing_prices() -> anyhow::Result<()> {
    ...
    // Update velox's oracle price. It isn't used in this test but is required
    ...
    // Price: 99_999
    // Volume in VELOX: 1 + 1 (from previous block) = 2
    ...
    // Price: 100_001
    // Volume in VELOX: 1 + 2 (from previous blocks) = 3
...
async fn index_pair_prices_with_small_amounts() -> anyhow::Result<()> {
    ...
    // - limit, sell, price 100000 USDC per VELOX, size 2 VELOX
    // - limit, buy, price 100000, size 1 VELOX
    // - market, buy, size 1 VELOX
    suite
        .execute(
            &mut accounts.user1,
            contracts.dex,
            ...
            coins! {
                velox::DENOM.clone() => Uint128::new(20000000000000),
