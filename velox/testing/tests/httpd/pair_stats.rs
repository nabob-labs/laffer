use {
    ...
    velox_testing::{TestAccounts, TestOption, TestSuiteWithIndexer, setup_test_with_indexer},
    velox_types::{
    ...
    local_set
        .run_until(async {
            tokio::task::spawn_local(async move {
                ...
                assert_eq!(response.base_denom, "velox");
    ...
    local_set
        .run_until(async {
            tokio::task::spawn_local(async move {
                ...
                // Find the velox/usdc pair
                let velox_usdc = response
                    .iter()
                    .find(|p| p.base_denom == "velox" && p.quote_denom == "bridge/usdc");

                assert!(
                    velox_usdc.is_some(),
                    "Expected velox/usdc pair in allPairStats"
                );

                let pair = velox_usdc.unwrap();
...
async fn query_pair_stats_formats_small_prices_without_scientific_notation() -> anyhow::Result<()> {
    ...
    suite
        .execute(
            ...
            Coins::try_from([
                Coin::new(velox::DENOM.clone(), Uint128::new(20_000_000_000_000))?,
