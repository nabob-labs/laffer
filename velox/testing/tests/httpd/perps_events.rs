...
async fn event_by_addresses_drops_out_of_order_block() -> anyhow::Result<()> {
    use {
        ...
        velox_indexer_sql::entity::events::{EventStatus, Model as EventModel},
        velox_primitives::{Addr, FlatCommitmentStatus, Timestamp},
    };
    ...
    let pubsub = velox_httpd_context.pubsub.clone();
    let event_cache = velox_httpd_context.sql_context.event_cache.clone();
    let latest = velox_httpd_context
        .stream_context
        .perps()
        .tip()
