use {
    std::sync::Arc,
    velox_app::{AppError, CHAIN_ID, CONFIG, CONTRACTS, GasTracker, HaltReason, TraceOption},
    velox_math::{Bytable, NextNumber, Uint128, Uint256},
    velox_primitives::{
...
async fn upgrading_with_calling_contract() {
    mod new_bank {
        use {
            velox_math::{NextNumber, Uint256},
            velox_primitives::{Addr, Denom, Empty, MutableCtx, Order, Response, StdResult},
            velox_storage::Map,
        };
    ...
    suite
        .query_wasm_raw(
            bank,
            velox_bank::BALANCES.path((&accounts.owner.address(), &denom)),
        )
    ...
    suite.app.set_cargo_version_and_upgrade_handler(
        ...
        Some(|mut storage, vm, block| {
            ...
            let new_bank_code = ContractBuilder::new(Box::new(velox_bank::instantiate))
                .with_execute(Box::new(new_bank::execute))
                .build();

            // Update the bank contract's code.
            velox_app::CODES.update(&mut storage, bank_contract.code_hash, |mut code| {
