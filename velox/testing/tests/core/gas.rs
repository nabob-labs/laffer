use {
    test_case::test_case,
    velox_genesis::{AccountOption, GenesisOption, GenesisUser, GrugOption},
    velox_math::{Udec128, Uint128},
    velox_primitives::{Addressable, Coins, Duration, HashExt, Message, ResultExt},
    velox_testing::{
        Preset, TestOption, owner, setup_test_naive_with_custom_genesis, user1, user2, user3,
        user4, user5, user6, user7, user8, user9,
    },
    velox_types::{account_factory::NewUserSalt, auth::Key, constants::velox},
};
...
/// Build a custom genesis where the gas token is VELOX and the gas fee rate is
...
fn gas_genesis(sender_balance: u128) -> GenesisOption {
    let genesis_user = |key: velox_types::auth::Key, seed: u32, balance: u128| GenesisUser {
        salt: NewUserSalt {
            key,
            key_hash: match &key {
        ...
        velox_balance: Uint128::new(balance),
    };

    GenesisOption {
        grug: GrugOption {
            owner_index: 0,
            gas_token: velox::DENOM.clone(),
...
async fn gas_fee_charging_works(
    ...
    let outcome = suite
        .send_message_with_gas(
            &mut accounts.user1,
            gas_limit,
            Message::transfer(to, Coins::one(velox::DENOM.clone(), send_amount).unwrap()).unwrap(),
        )
        .await;
    ...
    suite
        .query_balance(&accounts.owner, velox::DENOM.clone())
        .should_succeed_and_equal(Uint128::new(owner_balance_after));
    suite
        .query_balance(&accounts.user1, velox::DENOM.clone())
        .should_succeed_and_equal(Uint128::new(sender_balance_after));
    suite
        .query_balance(&accounts.user9, velox::DENOM.clone())
