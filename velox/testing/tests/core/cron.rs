use {
    ...
    velox_testing::{ContractBuilder, TestOption, setup_test_naive},
    velox_types::constants::{eth, usdc, velox},
};
...
mod tester {
    use {
        ...
        velox_primitives::{Addr, Coin, Message, MutableCtx, Response, StdResult, SudoCtx},
        velox_storage::Item,
    };
...
mod failing_tester {
    use {
        velox_math::{Number, NumberConst, Uint128},
        velox_primitives::{Empty, MutableCtx, Response, StdResult, SudoCtx},
    };
...
async fn cronjob_works() {
    ...
    let cron3 = suite
        .instantiate(
            ...
            &tester::Job {
                receiver,
                coin: Coin::new(velox::DENOM.clone(), 1).unwrap(),
            },
            "cron3",
            Some("cron3"),
            None,
            coins! { velox::DENOM.clone() => 3 },
        )
        .await
        .should_succeed()
        .address;
    ...
    // - cron3 (velox, interval 3): scheduled at T+3
    //
    ...
    for balances in [
        ...
        // cron3 sends 1 velox (2 remaining), rescheduled to T+6
        Balances {
            usdc: 2,
            eth: 1,
            velox: 1,
        ...
        // cron3 sends 1 velox (1 remaining), rescheduled to T+9
        Balances {
            usdc: 2,
            eth: 3,
            velox: 2,
        ...
        //
        // cron3 sends 1 velox (0 remaining, runs out)
        Balances {
            usdc: 2,
            eth: 3,
            velox: 3,
        ...
        suite
            .query_balance(&accounts.user1, velox::DENOM.clone())
            .should_succeed_and_equal(initial_velox + balances.velox.into());
    }
}
