use {
    ...
    velox_primitives::{Addressable, Denom, Inner},
};
        ...
        old_balances
            ...
            .map(|denom| {
                ...
                let change = match new_balance.cmp(&old_balance) {
                    Ordering::Greater => {
                        BalanceChange::Increased((new_balance - old_balance).into_inner())
                    }
                    Ordering::Less => {
                        BalanceChange::Decreased((old_balance - new_balance).into_inner())
                    }
