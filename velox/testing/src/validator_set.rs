use {
    ...
    velox_primitives::{Addr, Hash256, HashExt, HexBinary, HexByteArray, Inner, Shared},
};
...
impl MockValidatorSet {
    ...
    where
        I: IntoIterator<Item = velox_eth_utils::SigningKey>,
    {
    ...
    /// submitted to the Mailbox contract on Velox for processing.
    pub fn sign(
        &self,
        sender: Addr32,
        destination_domain: Domain, // This should be the domain of Velox.
        recipient: Addr,
        body: HexBinary,
        ...
        let signatures = self
            ...
            .iter()
            .map(|sk| {
                let signature = velox_eth_utils::sign_digest(multisig_hash.into_inner(), sk);
    ...
    pub fn validator_addresses(&self) -> BTreeSet<HexByteArray<20>> {
        self.validators
            .iter()
            .map(|sk| HexByteArray::from_inner(velox_eth_utils::derive_address(sk.verifying_key())))
            .collect()
    }
