# Purity rule for `velox-perps` internal functions

**TL;DR**: Internal business-logic functions in `velox-perps` that take
...
Trader-facing handlers in `velox-perps` are structured in three layers.
...
- **intermediate** (`_`-prefixed): takes individual components of
  ...
  outer calls several intermediates in a loop, and the bolt `Buffer`
...
- **`&mut dyn Storage`** — bolt handles transaction-level rollback at
...
When adding a new public-within-module function to `velox-perps`, ask:
