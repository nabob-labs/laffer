use {
    ...
    anyhow::ensure,
    velox_types::perps::{CancelOrderRequest, SubmitOrCancelOrderRequest, SubmitOrderRequest},
    bolt::{EventBuilder, Inner, MutableCtx, NonEmpty, Response},
};
...
/// (bolt's `Buffer` makes in-call writes visible to subsequent reads).
/// If any action returns `Err`, the `?` propagation bubbles the error
/// out of `execute`, bolt drops the buffer without flushing, and every
