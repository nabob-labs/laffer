...
//! reuses the node's `velox_indexer_metrics::run_metrics_server` and is
...
pub fn install() -> anyhow::Result<PrometheusHandle> {
    ...
    velox_archive_block_source::init_metrics();
    velox_archive_projection::init_metrics();
    velox_archive_app::init_metrics();
    velox_archive_httpd::init_metrics();
    ...
    metrics::gauge!(
        ...
        "commit" => velox_primitives::GIT_COMMIT,
    )
    .set(1.0);
...
/// on the current task with no extra thread — the same way the velox node drives
/// it). Only called when `cfg.enabled`.
pub async fn serve(cfg: &MetricsConfig, handle: PrometheusHandle) -> anyhow::Result<()> {
    velox_indexer_metrics::run_metrics_server(&cfg.ip, cfg.port, handle)
        .await
