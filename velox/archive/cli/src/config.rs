...
//! [`velox_config_parser::parse_config`], which layers **environment
//! overrides** on top: any field is settable via `SECTION__FIELD`
//! (`__`-separated, e.g. `POSTGRES__URL`, `BLOCK_SOURCE__LIVE_URL`), mirroring
//! `velox-cli`. So a deployment can keep secrets (the Postgres URL) out of the
...
use {
    serde::Deserialize,
    std::path::PathBuf,
    velox_archive_projection::{EventType, WhiteOrBlackList},
};
...
pub struct Config {
    /// Tracing filter directive, e.g. `info`, `debug`, `warn,velox=debug`.
...
pub enum BlockSourceConfig {
    ...
    /// Co-located with a velox node: reads its indexer cache and httpd.
...
pub struct MetricsConfig {
    ...
    pub ip: String,
    /// Port to bind. Default `9191`, matching the velox node's metrics-port
    /// convention.
...
#[cfg(test)]
mod tests {
    use {super::*, std::collections::HashSet, velox_config_parser::parse_config};
    ...
    /// `velox-cli` behaviour we must preserve. Doubling as a guard that the
