...
use {
    ...
    velox_archive_httpd::ApiError,
    velox_primitives::{Addr, Hash256, Timestamp},
};
...
/// The kind of an executed unit — mirrors [`velox_primitives::FlatCategory`].
...
impl UnitKind {
    ...
    pub(crate) fn code(self) -> i16 {
        match self {
            Self::Cron => velox_primitives::FlatCategory::Cron as i16,
            Self::Transaction => velox_primitives::FlatCategory::Tx as i16,
        }
    }

    fn from_code(code: i16) -> Result<Self, ApiError> {
        match code {
            c if c == velox_primitives::FlatCategory::Cron as i16 => Ok(Self::Cron),
            c if c == velox_primitives::FlatCategory::Tx as i16 => Ok(Self::Transaction),
...
///
/// [`TxOutcome`]: velox_primitives::TxOutcome
/// [`CronOutcome`]: velox_primitives::CronOutcome
#[derive(Serialize)]
#[serde(rename_all = "snake_case")]
pub(crate) enum UnitOutcome {
    ...
    Transaction(Box<velox_primitives::TxOutcome>),
    Cron(Box<velox_primitives::CronOutcome>),
}
