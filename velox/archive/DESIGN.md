...
external service that consumes finalized blocks from the velox chain and
...
from the velox chain binary, replacing today's in-process `indexer/sql/` +
...
in-process indexer (`velox/indexer/sql`).

```plain
velox/archive/
├── types/         → lib `velox-archive-types`
│                     • shared wire types (BlockData, ...)
│                     • no deps on other archive crates
├── block-source/  → lib `velox-archive-block-source`
│                     • trait BlockSource
│                     • concrete impls (LocalBlockSource, ...)
│                     • deps: types
├── projection/    → lib `velox-archive-projection`
...
pub type BlockData = velox_primitives::FullBlock;
```
...
- [`LocalBlockSource`](./design/local-block-source.md) — V1. Follows the
  in-process `velox-httpd` GraphQL `full_block` subscription for the live tail
...
| ------------------- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **V1** — Co-located | [`LocalBlockSource`](./design/local-block-source.md)   | Reuses the velox node's GraphQL `full_block` subscription (live tail) + cache files on disk (catch-up). No new storage.                                                                                                                                          |
