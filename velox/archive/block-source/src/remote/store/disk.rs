use {
    ...
    velox_archive_types::{AnyResult, BlockData},
};
...
impl BlockStore for RocksdbBlockStore {
    ...
    /// as gauges — the same shape as the velox node's `StatisticsWorker`, under
...
/// sampled per column family, grouped into the velox `rocksdb_*` gauges. Raw
...
#[cfg(test)]
mod tests {
    use {super::*, velox_archive_types::BlockDataExt, velox_temp_rocksdb::TempDataDir};

    fn block(height: u64) -> BlockData {
        use velox_primitives::{Block, BlockInfo, BlockOutcome, Hash256, Timestamp};
        BlockData {
            block: Block {
                info: BlockInfo {
                    height,
