...
/// Thin client against a velox node's `httpd` — the multiplexed WebSocket `/ws`
...
/// implementation. The local source points it at the in-process `velox-httpd`,
...
impl HttpdClient {
    ...
    /// against the in-process `velox-httpd`, the remote one against a sentinel.
