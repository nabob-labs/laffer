...
//! a mock Velox node feeding the `App` — so the integration tests in `tests/`
...
use {
    ...
    velox_archive_app::{App, PgChCommitter},
    velox_archive_block_source::{
    ...
    velox_archive_httpd::HttpdConfig,
    velox_archive_projection::{ActivityProjection, Committer, Projection},
    velox_genesis::{Contracts, GenesisOption},
    velox_primitives::{
...
/// `velox-testing`). The database is created if missing; each test carves its
...
async fn base_engine() -> anyhow::Result<String> {
    ...
    // CI points `DATABASE_URL` at a `velox_test` database the bare Postgres
...
///
/// CI points `DATABASE_URL` at a `velox_test` database that the bare Postgres
...
/// A full archive environment: a mock Velox node producing blocks,
...
/// Spin up an in-process mock Velox node (real chain + httpd) on a free port,
