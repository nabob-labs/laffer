...
//! `VELOX_SECRET_KEY` (32-byte secp256k1 secret, hex) and `VELOX_ACCOUNT_ADDRESS`
//! (the funded account). The account must have perps margin on the target chain.
//!
//! Run with:
//!
//! ```sh
//! VELOX_SECRET_KEY=... VELOX_ACCOUNT_ADDRESS=0x... \
//!   cargo run -p velox-sdk --example native_perps_latency
//! ```
...
use {
    ...
    velox_primitives::{Addr, Coins, EventName, Message, NonEmpty, QueryClientExt, Signer},
    velox_sdk::{
...
fn sign_submit_order(
    ...
    chain_id: &str,
    perps: Addr,
    pair_id: &velox_order_book::PairId,
    limit_price: UsdPrice,
    coid: ClientOrderId,
) -> Result<velox_primitives::Tx> {
...
async fn main() -> Result<()> {
    ...
    let secret = {
        let secret_hex = env::var("VELOX_SECRET_KEY")
            .map_err(|_| anyhow!("set VELOX_SECRET_KEY to the 32-byte secp256k1 secret (hex)"))?;
        let secret_bytes: [u8; 32] = hex::decode(secret_hex.trim().trim_start_matches("0x"))?
            .try_into()
            .map_err(|_| anyhow!("VELOX_SECRET_KEY must be exactly 32 bytes of hex"))?;
        Secp256k1::from_bytes(secret_bytes)?
    };
    let address = {
        let hex = env::var("VELOX_ACCOUNT_ADDRESS")
            .map_err(|_| anyhow!("set VELOX_ACCOUNT_ADDRESS to the funded account address"))?;
