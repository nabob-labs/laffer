...
pub trait Secret: Sized {
    ...
    /// Return the [`Key`](velox_types::auth::Key) for using in the Velox account factory.
    fn key(&self) -> Key;

    /// Return the key hash for use in the Velox account factory.
...
pub struct Eip712 {
    inner: Secp256k1,
    // This means the Ethereum address, not the Velox address.
    pub address: velox_eth_utils::Address,
}
...
impl Secret for Eip712 {
    ...
    fn key(&self) -> velox_types::auth::Key {
        velox_types::auth::Key::Ethereum(self.address.into())
    }
    ...
    fn sign_transaction(&self, sign_doc: SignDoc) -> anyhow::Result<Signature> {
        ...
        Ok(Signature::Eip712(Eip712Signature {
            ...
            sig: velox_eth_utils::pack_signature(signature, recovery_id).into(),
        }))
    }
}
...
impl From<Secp256k1> for Eip712 {
    fn from(inner: Secp256k1) -> Self {
        let address = velox_eth_utils::derive_address(inner.inner.verifying_key());
        Self { inner, address }
    }
}
