...
/// Create the Velox genesis state given a genesis config.
    ...
    T: Into<Binary>,
{
    ...
    let owner = addresses[opt.bolt.owner_index as usize];
    ...
    let balances = opt
        ...
        .filter_map(|(user, address)| {
            if user.velox_balance.is_non_zero() {
                Some((*address, coins! {
                    velox::DENOM.clone() => user.velox_balance,
                }))
            } else {
                None
            }
    ...
    let config = Config {
        ...
        max_orphan_age: opt.bolt.max_orphan_age,
    };
