    ...
    G: Clone + 'static,
{
    Box::new(move |cfg: &mut ServiceConfig| {
        ...
        if let Some(static_path) = &velox_httpd_context.static_files_path {
            #[cfg(feature = "tracing")]
        ...
        service_config
            ...
            .app_data(web::Data::new(velox_httpd_context.db.clone()))
            .app_data(web::Data::new(velox_httpd_context.clone()))
            .app_data(web::Data::new(
                velox_httpd_context.indexer_httpd_context.clone(),
            ))
            .app_data(web::Data::new(
                velox_httpd_context.indexer_httpd_context.base.clone(),
            ))
            .app_data(web::Data::new(graphql_schema.clone()));
    })
}

/// Run the velox HTTP server with velox-specific context.
...
pub async fn run_server(
    ...
    velox_httpd_context: crate::context::Context,
    shutdown_flag: Arc<AtomicBool>,
    port_sender: Option<mpsc::Sender<u16>>,
) -> Result<(), indexer_httpd::error::Error> {
    let graphql_schema = crate::graphql::build_schema(velox_httpd_context.clone());

    #[cfg(feature = "tracing")]
    tracing::info!(
        httpd_config.ip,
        httpd_config.port,
        "Starting velox httpd server"
    );
