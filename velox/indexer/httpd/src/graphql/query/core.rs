use {
    ...
    async_graphql::*,
    velox_primitives::{Query, QueryResponse, TxOutcome, UnsignedTx},
};
#[cfg(feature = "metrics")]
...
impl CoreQuery {
    pub async fn _query_app(
        ...
        let (response, block_height) = app_ctx.velox_app.query_app(request).await?;
    ...
    pub async fn _query_status(app_ctx: &MinimalContext) -> Result<Status, Error> {
        ...
        let status = Status {
            block: app_ctx.velox_app.last_finalized_block().await?.into(),
            chain_id: app_ctx.velox_app.chain_id().await?,
        };
    ...
    pub async fn _simulate(app_ctx: &MinimalContext, tx: UnsignedTx) -> Result<TxOutcome, Error> {
        ...
        let outcome = app_ctx.velox_app.simulate(tx).await?;
