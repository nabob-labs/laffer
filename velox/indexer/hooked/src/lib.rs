use {
    ...
    velox_app::{APP_CONFIG, CONFIG, Indexer, IndexerResult, LAST_FINALIZED_BLOCK},
    velox_primitives::{Block, BlockOutcome, Config, Json, Storage},
};

// Re-export for convenience
pub use velox_app::IndexerError;

/// Composite indexer that owns the three production indexer components and
/// orchestrates their per-block work as a single `velox_app::Indexer`.
///
...
pub struct HookedIndexer {
    ...
    /// that node.
    pub stream: velox_indexer_stream::Indexer,
...
impl HookedIndexer {
    pub fn new(
        file: velox_indexer_cache::Cache,
        sql: velox_indexer_sql::Indexer,
        clickhouse: velox_indexer_clickhouse::Indexer,
    ) -> Self {
        Self {
            file,
            sql,
            clickhouse,
            stream: velox_indexer_stream::Indexer::new(
                velox_indexer_stream::DEFAULT_BLOCK_RING_CAPACITY,
                velox_indexer_stream::DEFAULT_PERPS_RING_CAPACITY,
            ),
