use {
    ...
    bolt::{Denom, Udec128_6, Udec128_24},
...
pub mod dec {
    ...
    pub fn deserialize<'de, D, U, const S: u32>(
        deserializer: D,
    ) -> Result<bolt::Dec<U, S>, D::Error>
    where
        D: Deserializer<'de>,
        U: de::Deserialize<'de>,
    {
        let inner: U = <_ as de::Deserialize<'de>>::deserialize(deserializer)?;
        let uint = bolt::Int::new(inner);
        let dec = bolt::Dec::raw(uint);
        Ok(dec)
    }
}
...
mod test {
    use {
        ...
        bnum::cast::CastFrom,
        chrono::SubsecRound,
        bolt::{Dec128_6, NumberConst, Udec128, Udec256, Uint128, Uint256},
    };
