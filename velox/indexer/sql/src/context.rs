use {
    crate::{
        entity::perps_trade::PerpsTrade, event_cache::EventCacheWriter, grug_migration,
        pubsub::PubSub, velox_migration, write::perps_trades::PerpsTradeCache,
    },
...
impl Context {
    /// Run both the grug-side and velox-side sea-orm migrators sequentially.
    /// Each migrator maintains its own physical migration table
    /// (`grug_seaql_migrations` and `velox_seaql_migrations`).
    pub async fn migrate_db(&self) -> Result<(), sea_orm::DbErr> {
        grug_migration::Migrator::up(&self.db, None).await?;
        velox_migration::Migrator::up(&self.db, None).await?;
        Ok(())
    }
