use {
    ...
    uuid::Uuid,
    velox_primitives::{FlatCommitmentStatus, FlatEvent, FlatEventStatus, FlatEvtTransfer},
};
#[cfg(feature = "metrics")]
use {
...
pub(crate) async fn save_transfers(
    ...
    #[cfg(feature = "metrics")]
    counter!("indexer.velox.hooks.transfer_events.total").increment(transfer_events.len() as u64);
    ...
    #[cfg(feature = "metrics")]
    metrics::counter!("indexer.velox.hooks.transfers.total").increment(new_transfers.len() as u64);
    ...
    #[cfg(feature = "metrics")]
    histogram!("indexer.velox.hooks.transfers.duration").record(start.elapsed().as_secs_f64());
...
pub fn init_metrics() {
    ...
    describe_histogram!(
        "indexer.velox.hooks.transfers.duration",
        "Transfer hook duration in seconds"
    );

    describe_counter!(
        "indexer.velox.hooks.transfer_events.total",
        "Total transfer events processed"
    );

    describe_counter!(
        "indexer.velox.hooks.transfers.total",
        "Total transfers created"
    );

    describe_counter!(
        "indexer.velox.hooks.transfers.errors.total",
