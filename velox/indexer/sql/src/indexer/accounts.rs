use {
    ...
    bolt::{BlockAndBlockOutcomeWithHttpDetails, EventName, Json, JsonDeExt},
    bolt_types::{FlatCommitmentStatus, FlatEvent, SearchEvent},
...
pub(crate) async fn save_accounts(
    ...
    #[cfg(feature = "metrics")]
    {
        counter!("indexer.velox.hooks.users_registered.total")
            .increment(user_registered_events.len() as u64);
        counter!("indexer.velox.hooks.accounts_registered.total")
            .increment(account_registered_events.len() as u64);
        counter!("indexer.velox.hooks.keys_added.total")
            .increment(account_key_added_events.len() as u64);
        counter!("indexer.velox.hooks.keys_removed.total")
    ...
    if !account_registered_events.is_empty() {
        ...
        for (account_registered_event, tx_hash) in account_registered_events {
                ...
                .map(|user| user.id)
            {
                ...
                #[cfg(feature = "metrics")]
                counter!("indexer.velox.hooks.accounts.total").increment(1);
    ...
    #[cfg(feature = "metrics")]
    histogram!("indexer.velox.hooks.accounts.duration").record(start.elapsed().as_secs_f64());
...
pub fn init_metrics() {
    describe_histogram!(
        "indexer.velox.hooks.accounts.duration",
        "Account hook duration in seconds"
    );

    describe_counter!(
        "indexer.velox.hooks.accounts.total",
        "Total account hook executions"
    );

    describe_counter!(
        "indexer.velox.hooks.users_registered.total",
        "Total users registered"
    );

    describe_counter!(
        "indexer.velox.hooks.accounts_registered.total",
        "Total accounts registered"
    );

    describe_counter!("indexer.velox.hooks.keys_added.total", "Total keys added");

    describe_counter!(
        "indexer.velox.hooks.keys_removed.total",
        "Total keys removed"
    );

    describe_counter!(
        "indexer.velox.hooks.accounts.errors.total",
