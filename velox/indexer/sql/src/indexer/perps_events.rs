...
pub(crate) async fn save_perps_events(
    ...
    #[cfg(feature = "metrics")]
    counter!("indexer.velox.hooks.perps_events.total").increment(events.len() as u64);
    ...
    #[cfg(feature = "metrics")]
    histogram!("indexer.velox.hooks.perps_events.duration").record(start.elapsed().as_secs_f64());
...
fn try_build_perps_trade(
    event: &bolt_types::CheckedContractEvent,
    block_height: u64,
    created_at: &str,
    trade_idx: u32,
) -> Option<PerpsTrade> {
...
pub fn init_metrics() {
    describe_histogram!(
        "indexer.velox.hooks.perps_events.duration",
        "Perps events hook duration in seconds"
    );

    describe_counter!(
        "indexer.velox.hooks.perps_events.total",
        "Total perps events processed"
    );

    describe_counter!(
        "indexer.velox.hooks.perps_events.errors.total",
