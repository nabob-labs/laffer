...
impl Indexer {
    ...
    pub async fn start(&mut self, _storage: &dyn Storage) -> velox_app::IndexerResult<()> {
        ...
        self.context
            .migrate_db()
            .await
            .map_err(|e| velox_app::IndexerError::database(e.to_string()))?;
    ...
    pub async fn post_indexing(
        ...
        // 1. Velox-side writes (blocks, transactions, messages, events).
        ...
        //    indexer pipeline. The velox-side writes below depend on the rows
        ...
        // 2. Velox-side writes (transfers, accounts, perps_events).
        ...
        // Velox-side write failures are logged and counted but do not abort
        ...
        // is indexed at the grug level" notifications even if a velox-side
        ...
        if let Err(_err) = transfers_result {
            ...
            #[cfg(feature = "metrics")]
            metrics::counter!("indexer.velox.hooks.transfers.errors.total").increment(1);
        }
        if let Err(_err) = accounts_result {
            ...
            #[cfg(feature = "metrics")]
            metrics::counter!("indexer.velox.hooks.accounts.errors.total").increment(1);
        }
        if let Err(_err) = perps_result {
            ...
            #[cfg(feature = "metrics")]
            metrics::counter!("indexer.velox.hooks.perps_events.errors.total").increment(1);
        }

        // 3. Single pubsub publish, after grug-side and velox-side writes have
        //    been attempted. Fires whether velox-side writes succeeded or not.
...
#[cfg(test)]
mod tests {
    use {super::*, velox_primitives::MockStorage};

    /// This is when used from Velox, which is async. In such case the indexer does not have its
