...
struct Cli {
    /// Directory for the physical database [default: ~/.velox]
...
async fn main() -> anyhow::Result<()> {
    ...
    let velox_network = non_empty_env("VELOX_NETWORK");
    ...
    let otel_layer_opt = if cfg.trace.enabled {
        ...
        if let Some(velox_network) = velox_network.clone() {
            attrs.push(KeyValue::new("velox_network", velox_network));
        }

        let resource = Resource::builder()
            .with_service_name("velox")
        ...
        let tracer = provider.tracer("velox");
    ...
    let sentry_layer = if cfg.sentry.enabled {
        ...
        sentry::configure_scope(|scope| {
            ...
            if let Some(velox_network) = &velox_network {
                scope.set_tag("velox_network", velox_network);
            }
