...
impl StartCmd {
    ...
    /// Setup the hooked indexer with both SQL and Velox indexers, and prepare contexts for HTTP servers
    ...
    async fn run_velox_httpd_server(
        ...
        let warmup_ctx = velox_httpd_context.clone();
        tokio::spawn(async move {
            let warmup_start = std::time::Instant::now();
            metrics::gauge!("velox_httpd.cache_warmup.in_progress").set(1.0);

            tracing::info!("Starting velox HTTP cache warm-up");
            ...
            metrics::gauge!("velox_httpd.cache_warmup.in_progress").set(0.0);
            metrics::histogram!("velox_httpd.cache_warmup.duration_seconds")
                .record(elapsed.as_secs_f64());

            tracing::info!(
                elapsed_secs = elapsed.as_secs_f64(),
                "Velox HTTP cache warm-up complete"
            );
        });
        ...
        ID: Indexer + Send + Sync + 'static,
    {
        ...
        if halt_rx.borrow().is_some() {
            ...
            // (`run_velox_httpd_server`, `run_metrics_httpd_server`) rely on
