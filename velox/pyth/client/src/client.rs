...
use {
    ...
    url::Url,
    velox_primitives::{Inner, Lengthy, NonEmpty},
    velox_pyth_types::{ExponentialBackoff, PriceUpdate, PythLazerSubscriptionDetails},
};
...
impl PythClient {
    ...
    async fn connect(
        ...
        #[cfg(feature = "metrics")]
        counter!(velox_pyth_types::metrics::PYTH_RECONNECTION_ATTEMPTS).increment(1);
    ...
    fn analyze_data(
        ...
        subscriptions_data: &mut HashMap<u64, velox_pyth_types::LeEcdsaMessage>,
        ...
        for data in received_data.drain(..) {
            match data {
                AnyResponse::Binary(update) => {
                    ...
                    #[cfg(feature = "metrics")]
                    histogram!(velox_pyth_types::metrics::PYTH_MESSAGES_RECEIVED)
        ...
        let stream = stream! {
            loop {
                ...
                if to_resubscribe {
                    ...
                    #[cfg(feature = "metrics")]
                    histogram!(velox_pyth_types::metrics::PYTH_UPTIME).record(uptime.as_secs_f64());
                    ...
                    loop {
                            ...
                            .await
                        {
                            Ok(()) => {
                                ...
                                #[cfg(feature = "metrics")]
                                histogram!(velox_pyth_types::metrics::PYTH_RECONNECTION_TIME)
                ...
                #[cfg(feature = "metrics")]
                counter!(velox_pyth_types::metrics::PYTH_DATA_READ).increment(1);
...
mod tests {
    use {
        ...
        velox_pyth_types::PythLazerSubscriptionDetails,
    };
