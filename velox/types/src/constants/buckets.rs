...
pub mod velox_usdc {
    use {
        super::*,
        crate::constants::{velox, usdc},
    };

    pub const ONE_THOUSANDTH: NonZero<Price> = bucket(1, 1000, velox::DECIMAL, usdc::DECIMAL);

    pub const ONE_HUNDREDTH: NonZero<Price> = bucket(1, 100, velox::DECIMAL, usdc::DECIMAL);

    pub const ONE_TENTH: NonZero<Price> = bucket(1, 10, velox::DECIMAL, usdc::DECIMAL);

    pub const ONE: NonZero<Price> = bucket(1, 1, velox::DECIMAL, usdc::DECIMAL);

    pub const TEN: NonZero<Price> = bucket(10, 1, velox::DECIMAL, usdc::DECIMAL);

    pub const FIFTY: NonZero<Price> = bucket(50, 1, velox::DECIMAL, usdc::DECIMAL);

    pub const ONE_HUNDRED: NonZero<Price> = bucket(100, 1, velox::DECIMAL, usdc::DECIMAL);
}
