...
use {
    ...
    velox_primitives::{Addr, EventBuilder, Order as IterationOrder, StdResult, Storage},
    velox_types::perps::UserState,
};
...
/// `velox/exchange/perps/purity.md`.
#[derive(Debug)]
...
pub fn compute_resize_reduce_only_outcome(
    ...
    for (order_key, order) in closing {
        ...
        if alloc.is_zero() {
            ...
            let book = if is_bid { BIDS } else { ASKS };
...
mod tests {
    use {
        ...
        velox_math::Uint64,
        velox_order_book::{FundingPerUnit, UsdPrice, UsdValue},
        velox_primitives::{MockContext, Timestamp},
        velox_types::perps::{PairParam, Position},
    };
