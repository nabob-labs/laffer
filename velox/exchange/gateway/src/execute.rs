use {
    ...
    velox_math::{IsZero, Number, NumberConst, Uint128},
    velox_primitives::{
...
fn receive_remote(
    ...
    // First,
    // - if the token is not native on Velox, mint it to the Gateway contract;
    ...
    // `receive` to be triggered; e.g. activating a new account (see `velox_auth::receive_transfer`).
    Ok(Response::new()
...
fn transfer_remote(ctx: MutableCtx, remote: Remote, recipient: Addr32) -> anyhow::Result<Response> {
    ...
    // 2. Burn the alloyed token to be transferred (only if the token is not native on Velox).
