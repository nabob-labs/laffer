...
pub mod account_factory {
    use {
        velox_storage::Map,
        velox_types::account_factory::{User, UserIndex},
    };
...
pub mod account {
    use {
        ...
        velox_types::auth::{AccountStatus, Nonce, SessionKey},
    };
...
/// this requirement.
///
/// Since Velox isn't an EVM chain and hence doesn't have an EIP-155 chain ID,
            ...
            let (verifying_contract, message) = match data {
                VerifyData::Transaction(sign_doc) => {
                    ...
                    ensure_resolver_declares_fields(
                        &resolver,
                        "Message",
                        &["sender", "data", "gas_limit", "messages"],
                    )?;
                ...
                VerifyData::Session(session_info) => {
                    ensure_resolver_declares_fields(
                        &resolver,
                        "Message",
                        &["chain_id", "expire_at", "session_key"],
                    )?;
...
///
/// ```bash
/// cargo run --example generate_test_data -p velox-auth
/// ```
///
