use {
    anyhow::bail,
    velox_genesis::{Codes, Contracts, GenesisCodes},
    velox_proposal_preparer::ProposalPreparer,
    velox_testing::{TestAccounts, setup_suite_with_db_and_vm},
    bolt_app::SimpleCommitment,
    bolt_db_memory::MemDb,
    bolt_testing::MockClient,
    bolt_types::HttpdConfig,
    bolt_vm_rust::{ContractWrapper, RustVm},
...
pub use {
    ...
    velox_testing::{BridgeOp, Preset, TestOption},
    bolt_testing::BlockCreation,
    ...
    let velox_context: velox_indexer_sql::context::Context = indexer
        ...
        .map_err(|e| {
            indexer_sql::error::IndexerError::from(anyhow::anyhow!(
                "Failed to create separate context for velox indexer: {e}",
            ))
        })?
        .into();
